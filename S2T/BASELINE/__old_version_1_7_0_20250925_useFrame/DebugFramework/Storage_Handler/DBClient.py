...
class DB_Client:
    ...
    def delete_last_updated(self, collection_name, query, sort_field="updated_at"):
        if self.collection_exists(collection_name):
            collection = self._db[collection_name]
            doc = collection.find_one(query, sort=[(sort_field, -1)])
            if doc:
    ...
    def filter_items(self, collection_name, filter_query):
        if self.collection_exists(collection_name):
            ...
            self.logger(f"Collection {collection_name} not found.")
            return []
    def update_item(self, collection_name, query, update_values):
        if self.collection_exists(collection_name):
            collection = self._db[collection_name]
            ...
            if result.matched_count > 0:
                if result.modified_count > 0:
                    self.logger(f"Item updated in collection {collection_name}.")
                else:
                    self.logger(f"Item matched but no changes were needed in {collection_name}.")
            else:
                self.logger(f"No item matching the query found in collection {collection_name}.")
        else:
            self.logger(f"Collection {collection_name} not found.")
