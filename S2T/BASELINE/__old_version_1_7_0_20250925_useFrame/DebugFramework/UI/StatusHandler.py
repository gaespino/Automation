...
class ExecutionStateMachine:
	...
	def _handle_execution_cancelled(self, data) -> Dict[str, Any]:
		"""Handle execution cancellation - UPDATED"""
		self.current_state = ExecutionState.CANCELLED
	...
	def _apply_ui_updates(self, updates: Dict[str, Any]):
		...
		try:
			...
			self._safe_update_wrapper('reset_thread_state', updates,
				lambda: setattr(self.ui, 'thread_active', False))
			...
			#		self.ui._coordinate_status_updates(label_config)					
			#		#label_config = updates['status_label']
			...
			#if 'update_progress' in updates and not getattr(self.ui, '_strategy_progress_active', False):
			...
			#	# Convert to status label format
			#	status_config = {'text': f" {updates['status_update']} "}
			#	self.ui._coordinate_status_updates(status_config)				
			...
			#	self.ui._coordinate_button_states('enable_after_completion')
