...
from ExecutionHandler.utils.PanelManager import ControlPanelThreadIntegration
importlib.reload(th)
...
class DebugFrameworkControlPanel:
	...
	def start_tests_thread(self):
		...
		# Update Framework Configuration Unit Data and Upload to DB
	...
	def _run_experiments_thread(self, experiments_list, s2t_config, options):
		...
		try:
			...
			if self.Framework:
				...
				if options['check_unit_data']: self.Framework.update_unit_data()
		...
		try:
			...
			if hasattr(self.Framework, 'execution_state'):
				if self.Framework.execution_state.is_cancelled():
					self.log_status(f"[CANCEL] Framework execution state shows cancellation for '{experiment_name}'")
		...
		try:
			for frame_data in self.experiment_frames:
				...
				if run_label.winfo_exists():
					...
					# Reset to clean idle state with animation
					self._animate_status_change(run_label, "Idle", "lightgray", "black", "reset_clean")
	...
	def _coordinate_button_states(self, update_type: str, button_data: Dict[str, Any] = None):
		"""Coordinate button state changes to prevent conflicts"""
			...
			if update_type == 'specific_button_update' and button_data:
				for button_name, config in button_data.items():
					if hasattr(self, button_name):
						try:
							getattr(self, button_name).configure(**config)
						except Exception as e:
	...
	def _coordinate_status_updates(self, update_data: Dict[str, Any]):
		...
		if current_priority >= last_priority:
			...
			# Reset priority after delay for lower priority updates
			if current_priority > 60:  # Only for high priority statuses
				self.root.after(3000, lambda: setattr(self, '_last_status_priority', 40))
		...
		try:
			for frame_data in self.experiment_frames:
				if frame_data['experiment_name'] == experiment_name:
					...
					if run_label.winfo_exists():
						# Enhanced animation based on status
						if status in ['In Progress', 'Running']:
							self._animate_status_change(run_label, status, bg_color, fg_color, 'pulse_continuous')
						elif status in ['Done', 'Complete', 'Success']:
							self._animate_status_change(run_label, status, bg_color, fg_color, 'success_flash')
						elif status in ['Fail', 'Failed', 'Error']:
							self._animate_status_change(run_label, status, bg_color, fg_color, 'error_flash')
						elif status in ['Cancelled', 'Cancel']:
							self._animate_status_change(run_label, status, bg_color, fg_color, 'cancel_fade')
						elif status in ['Halted', 'Paused']:
							self._animate_status_change(run_label, status, bg_color, fg_color, 'warning_pulse')
						elif status in ['Idle', 'Ready']:
							self._animate_status_change(run_label, status, bg_color, fg_color, 'reset_clean')
						else:
	...
	def _animate_status_change(self, label, status: str, bg_color: str, fg_color: str, 
							animation_type: str):
		"""Animate status changes for better visual feedback"""
			
		try:
			# Cancel any existing animation
			if hasattr(label, '_animation_after_id'):
				self.root.after_cancel(label._animation_after_id)
			
			if animation_type == 'pulse_continuous':
				# Continuous pulse for active states
				self._start_continuous_pulse(label, status, bg_color, fg_color)
				
			elif animation_type == 'success_flash':
				# Success animation with green flash
				self._animate_success_flash(label, status, bg_color, fg_color)
				
			elif animation_type == 'error_flash':
				# Error animation with red flash and shake effect
				self._animate_error_flash(label, status, bg_color, fg_color)
				
			elif animation_type == 'cancel_fade':
				# Cancel animation with fade to gray
				self._animate_cancel_fade(label, status, bg_color, fg_color)
				
			elif animation_type == 'warning_pulse':
				# Warning pulse for halted states
				self._animate_warning_pulse(label, status, bg_color, fg_color)
				
			elif animation_type == 'reset_clean':
				# Clean reset animation
				self._animate_reset_clean(label, status, bg_color, fg_color)
				
			else:
				# Default - no animation
				label.configure(text=status, bg=bg_color, fg=fg_color)
				
		except Exception as e:
			self.log_status(f"[ERROR] Animation failed: {e}")
	...
	def _start_continuous_pulse(self, label, status: str, bg_color: str, fg_color: str):
		"""Start continuous pulse animation for running states"""
		
		def pulse_cycle(step=0):
			try:
				if not label.winfo_exists():
					return
					
				# Check if status has changed (stop pulsing if so)
				current_text = label.cget('text')
				if current_text != status and current_text not in ['In Progress', 'Running']:
					return
				
				# Pulse cycle: normal -> bright -> normal
				if step % 4 == 0:
					label.configure(text=status, bg=bg_color, fg=fg_color)
				elif step % 4 == 1:
					# Slightly brighter
					bright_bg = self._brighten_color(bg_color, 1.2)
					label.configure(bg=bright_bg)
				elif step % 4 == 2:
					label.configure(text=status, bg=bg_color, fg=fg_color)
				else:
					# Slightly dimmer
					dim_bg = self._brighten_color(bg_color, 0.8)
					label.configure(bg=dim_bg)
				
				# Continue pulsing
				label._animation_after_id = self.root.after(500, lambda: pulse_cycle(step + 1))
	...
	def _animate_success_flash(self, label, status: str, bg_color: str, fg_color: str):
		...
		def flash_sequence(step=0):
			try:
				...
				if step == 0:
					# Flash bright green
					label.configure(text=status, bg='#00FF00', fg='black')
					label._animation_after_id = self.root.after(150, lambda: flash_sequence(1))
				elif step == 1:
					# Flash white
					label.configure(bg='white', fg='black')
					label._animation_after_id = self.root.after(150, lambda: flash_sequence(2))
				elif step == 2:
					# Flash bright green again
					label.configure(bg='#00FF00', fg='black')
					label._animation_after_id = self.root.after(150, lambda: flash_sequence(3))
				else:
					# Final color
					label.configure(text=status, bg=bg_color, fg=fg_color)
	...
	def _animate_error_flash(self, label, status: str, bg_color: str, fg_color: str):
		"""Error flash animation with shake effect"""
		...
		def flash_sequence(step=0):
			try:
				...
				if step < 6:  # 3 flash cycles
					if step % 2 == 0:
						# Flash bright red
						label.configure(text=status, bg='#FF0000', fg='white')
					else:
						# Flash dark red
						label.configure(bg='#800000', fg='white')
					
					label._animation_after_id = self.root.after(100, lambda: flash_sequence(step + 1))
				else:
					# Final color
					label.configure(text=status, bg=bg_color, fg=fg_color)
	...
	def _animate_cancel_fade(self, label, status: str, bg_color: str, fg_color: str):
		"""Cancel fade animation"""
		
		def fade_sequence(step=0):
			try:
				if not label.winfo_exists():
					return
					
				fade_colors = ['white', '#F0F0F0', '#E0E0E0', '#D0D0D0', '#C0C0C0', bg_color]
				
				if step < len(fade_colors):
					label.configure(text=status, bg=fade_colors[step], 
								fg='darkgray' if step < len(fade_colors)-1 else fg_color)
					label._animation_after_id = self.root.after(200, lambda: fade_sequence(step + 1))
	...
	def _animate_warning_pulse(self, label, status: str, bg_color: str, fg_color: str):
		"""Warning pulse animation for halted states"""
		
		def pulse_sequence(step=0):
			try:
				if not label.winfo_exists():
					return
					
				if step < 6:  # 3 pulse cycles
					if step % 2 == 0:
						label.configure(text=status, bg=bg_color, fg=fg_color)
					else:
						# Pulse with lighter orange
						label.configure(bg='#FFB347', fg='black')
					
					label._animation_after_id = self.root.after(300, lambda: pulse_sequence(step + 1))
				else:
					label.configure(text=status, bg=bg_color, fg=fg_color)
	...
	def _animate_reset_clean(self, label, status: str, bg_color: str, fg_color: str):
		"""Clean reset animation"""
		
		def reset_sequence(step=0):
			try:
				if not label.winfo_exists():
					return
				...
				if step == 0:
					# Brief white flash
					label.configure(text=status, bg='white', fg='black')
					label._animation_after_id = self.root.after(100, lambda: reset_sequence(1))
				else:
					# Final clean state
					label.configure(text=status, bg=bg_color, fg=fg_color)
	...
	def _brighten_color(self, color: str, factor: float) -> str:
		...
		try:
			# Simple color brightening for common colors
			color_map = {
				'#00008B': '#0000CD' if factor > 1 else '#000080',  # Blue variations
				'#006400': '#008000' if factor > 1 else '#004000',  # Green variations
				'yellow': '#FFFF80' if factor > 1 else '#CCCC00',   # Yellow variations
				'orange': '#FFB347' if factor > 1 else '#CC8800',   # Orange variations
				'gray': '#A0A0A0' if factor > 1 else '#606060',     # Gray variations
			}
	...
	def on_closing(self):
		...
		try:
			...
			# Clear Framework callback
			if self.Framework:
				self.Framework._status_reporter = None
				
			# CRITICAL: Cleanup Tkinter variables in main thread
