...
def external_fuses(external_fuses = None, bsformat = False, ):
	...
	fuses_computes_common = [f for f in external_fuses if '.computes.' in f]
	fuses_ios_common = [f for f in external_fuses if '.ios.' in f]

	# Process specific Compute fuses dynamically
	all_possible_computes = ['compute0', 'compute1', 'compute2', 'compute3', 'compute4', 'compute5', 'compute6', 'compute7']  # Extend as needed
	for compute in all_possible_computes:
		temp_fuses = [f for f in external_fuses if f'.{compute}.' in f]
		if temp_fuses:
			if compute in compute_range:
				compute_fuses[compute] = temp_fuses
			else:
				print(f'WARNING: Fuses for {compute} are included but system does not have {compute}. Fuses will NOT be applied.')

	# Process specific IO fuses dynamically
	all_possible_ios = ['io0', 'io1', 'io2', 'io3']  # Extend as needed
	for io in all_possible_ios:
		temp_fuses = [f for f in external_fuses if f'.{io}.' in f]
		if temp_fuses:
			if io in io_range:
				io_fuses[io] = temp_fuses
			else:
				print(f'WARNING: Fuses for {io} are included but system does not have {io}. Fuses will NOT be applied.')

	# Expand common CBB fuses to all available CBBs
	for fuse in fuses_computes_common:
		print(f'>>>  Expanding common Compute fuse to all Computes: {fuse}')
		for compute in compute_range:
			expanded_fuse = fuse.replace('.computes.', f'.{compute}.')
			compute_fuses[compute].append(expanded_fuse)

	# Expand common IO fuses to all available IOs
	for fuse in fuses_ios_common:
		print(f'>>>  Expanding common IO fuse to all IOs: {fuse}')
		for io in io_range:
			expanded_fuse = fuse.replace('.ios.', f'.{io}.')
			io_fuses[io].append(expanded_fuse)

	# Print specific fuses being added (only non-expanded ones)
	all_specific_fuses = []
	for fuse_list in list(compute_fuses.values()) + list(io_fuses.values()):
		all_specific_fuses.extend(fuse_list)

	for f in all_specific_fuses:
		is_expanded = False
		for common_fuse in fuses_computes_common + fuses_ios_common:
			if common_fuse.replace('.computes.', '.').replace('.ios.', '.') in f:
				is_expanded = True
				break
		if not is_expanded:
	...
	if bsformat:
		print(f'\n {"*"*5} Modifying fuses to be usable in a bootscript array for each die {"*"*5}\n')
		for compute in compute_range:
			if compute_fuses[compute]:
				compute_fuses[compute] = bs_fuse_fix(fuse_str=compute_fuses[compute], bases=[f'sv.socket0.{compute}.fuses.'])
		for io in io_range:
			if io_fuses[io]:
				io_fuses[io] = bs_fuse_fix(fuse_str=io_fuses[io], bases=[f'sv.socket0.{io}.fuses.'])

	# Build external_config dictionary with all units
	external_config = {}
	for compute in compute_range:
		external_config[compute] = compute_fuses[compute]
	for io in io_range:
		external_config[io] = io_fuses[io]
