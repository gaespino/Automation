...
class defeature():
	...
	def Test(self):
		...
		#if bootReady:
		serial = ser.teraterm(visual=self.visual, qdf=self.qdf, bucket=self.bucket, log=self.log_file_path, cmds=self.macro_files, tfolder=self.tfolder, test=test, ttime=self.ttime, tnum=self.tnum, DebugLog = self.Debuglog, chkcore = self.coreslice, content = self.content, host = self.host)
...
class Framework():
	...
	@staticmethod				
	def RecipeExecutor(data, extmask = None, summary=True):
			

		if data['Test Type'] == 'Loops':				
					Loops(	name = data['Test Name'], 
							tnumber = data['Test Number'], 
							content = data['Content'], 
							loops = data['Loops'], 
							visual = data['Visual ID'], 
							bucket = data['Bucket'],  
							coreslice = data['Check Core'], 
							fastboot = data['FastBoot'],
							host= data['IP Address'],
							...
							mask = data['Configuration (Mask)'], 
							target = data['Test Mode'].lower(), 
							pseudo = data['Pseudo Config'], 
							dis2CPM = int(data['Disable 2 Cores'],16) if (data['Disable 2 Cores'] != None) else None, 
							corelic = int(data['Core License'].split(":")[0]) if (data['Core License'] != None) else None, 
							ttime= data['Test Time'],  
							reset= data['Reset'], 
							resetonpass=data['Reset on PASS'], 
							extMask=extmask, 
							u600w=data['600W Unit'], 
							summary = summary,
							script_file=data['Scripts File'],
							macro_files=data['TTL Folder'],
						)
					
		elif data['Test Type'] == 'Sweep':	
					Sweep(	name = data['Test Name'], 
							tnumber = data['Test Number'], 
							content = data['Content'], 
	...
	@staticmethod
	def TTL_Test(visual, cmds, bucket = 'dummy', test = 'TTL_Macro_Validation', ttime=30, tnum=1):
		qdf = dpm.qdf_str()
		ser.start(visual=visual, qdf=qdf, bucket=bucket, cmds=cmds, test=test, ttime=ttime, tnum=tnum)
