...
class Experiment():
	...
	def content_selection(self):
		...
		BootBreaks = True if self.postcode_break != None else False
		...
		if self.content == 'Dragon': 
			...
			self.runLinux = True
		elif self.content == 'PYSVConsole' and not BootBreaks: 
			self.runPYSVConsole = True
		elif self.content == 'PYSVConsole' and BootBreaks: 
...
class Framework():
	...
	def upload_data(self):
		...
		if DATABASE_HANDLER_READY:
			try:
				...
				logger(' Failed updloading data to DantaDB -- Excpetion {e}',2)
	...
	@staticmethod
	def reboot_unit(waittime=60, u600w=False):
