...
## Version: 1.30 
## Last Update notes: Added the following features:
## - Added self-check code for bootscript and fastboot
## - Reworked fuses assignment to allow the usage of PPVC configurations
## - Fixed an issue with bootscript of GNR SP
## - Added Channel indication for memory Controllers in coresEnabled script tileview
## - Added Bitstring indication in coresEnabled script Tileview and some interface improvements as well
## - Reworked the SV/IPC refresh to allow improve timing and code efficiency
## -
## Revision: 1.2
## Date: 20/06/2024
...
## Edit: gabriel.espinoza.ballestero@intel.com
## 
## Script with the necessary tools to start S2T configurations, has all the necessary logic for masking and booting the unit with the selected configuration
## Tools can be used indendent of S2T main loop
...
class System2Tester():
	...
	def __init__(self, target, masks = None, coremask=None, slicemask=None, boot = True, ht_dis = False, dis_2CPM = None, fresh_state = True, readFuse = False, clusterCheck = True , fastboot = True, ppvc_fuses = None):
	...
	'''
		...
		if global_dry_run == False:
			...
			fuse_cmd_override_reset(fuse_cmd_array=_fuse_str, s2t=True)

			if (stop_after_mrc):

				_wait_for_post(AFTER_MRC_POST, sleeptime=MRC_POSTCODE_WT, timeout = MRC_POSTCODE_CHECK_COUNT)
			if (boot_postcode):

				_wait_for_post(BOOT_STOP_POSTCODE, sleeptime=BOOT_POSTCODE_WT, timeout = BOOT_POSTCODE_CHECK_COUNT, additional_postcode=LINUX_POST)
						
			else:
				_wait_for_post(EFI_POST, sleeptime=EFI_POSTCODE_WT, timeout = EFI_POSTCODE_CHECK_COUNT, additional_postcode=LINUX_POST)
			sv_refreshed = False
	...
	def bsCheck(self, boot_postcode, stop_after_mrc, bootcont, sv, ipc):
		...
		if (stop_after_mrc or boot_postcode):
			sv.socket0.io0.uncore.ubox.ncdecs.biosscratchpad6_cfg=AFTER_MRC_POST
			print("***********************************v********************************************")
			print(f"sv.socket0.io0.uncore.ubox.ncdecs.biosscratchpad6_cfg={AFTER_MRC_POST:#x}")
			print ("%s(curr_state='phase6_cpu_reset_break')" % bootcont)
...
def gen_product_bootstring(bootopt = '', compute_cofig = 'GNRUCC', segment = 'X3' , b_extra = '', _boot_disable_ia = '', _boot_disable_llc ='',fuse_string ='',_fuse_files_compute = '', _fuse_files_io =''):
	...
	if chip == 'SP':
		_boot_string = f'{bootopt}(fused_unit=True, enable_strap_checks=False,enable_pm=True {b_extra}, {_boot_disable_ia} {_boot_disable_llc} {fuse_string} fuse_files_compute=[{_fuse_files_compute}], fuse_files_io=[{_fuse_files_io}])'
		...
		_boot_string = f'{bootopt}(fused_unit=True, enable_strap_checks=False,compute_config="{compute_cofig}",enable_pm=True,segment="{segment}" {b_extra}, {_boot_disable_ia} {_boot_disable_llc} {fuse_string} fuse_files_compute=[{_fuse_files_compute}], fuse_files_io=[{_fuse_files_io}])'
...
def coresEnabled(coreslicemask=None, logical=False, die = None, skip = False, print_cores = True, print_llcs = True):
	'''
...
def check_user_cancel():
	cancel_check = cancel_flag != None
	if cancel_check:
		#print('Checking Cancel Status', cancel_check, cancel_flag.is_set())	
		if cancel_flag.is_set():
			print("Framework Execution interrupted by user. Exiting...",2)
...
def _wait_for_post(postcode, sleeptime = 3, timeout = 10, additional_postcode = None):
	done=False
