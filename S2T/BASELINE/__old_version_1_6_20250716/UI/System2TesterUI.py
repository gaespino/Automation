...
class QuickDefeatureTool:
	def __init__(self, root, s2t, mode = 'mesh', product = 'GNRAP'):
		...
		self.s2t = s2t
		self.features = s2t.features()
		self.variables = s2t.variables()
		self.core_type = s2t.core_type
		self.computes = [c.capitalize() for c in s2t.computes]
		self.validclass = s2t.validclass
		self.dis2cpm_valid = [v for k,v in s2t.dis2cpm_dict.items()]
		...
		self.license_data = {v:k for k,v in self.s2t.license_dict.items() }#{ "Don't set license":0,"SSE/128":1,"AVX2/256 Light":2, "AVX2/256 Heavy":3, "AVX3/512 Light":4, "AVX3/512 Heavy":5, "TMUL Light":6, "TMUL Heavy":7}
		self.mode = mode
		self.enabledCores = []
		...
		if self.mode == 'slice':
			core_label = "Core Fixed Voltage"
			mesh_label = "Mesh Fixed Voltage (CFC/HDC)"
		else:
			core_label = "Core vBumps (-0.2V to 0.2V)"
			mesh_label = "Mesh vBumps (-0.2V to 0.2V)"    			
...
#	s2t = ''
#	root = tk.Tk()
#	app = QuickDefeatureTool(root, s2t)
#	root.mainloop()
