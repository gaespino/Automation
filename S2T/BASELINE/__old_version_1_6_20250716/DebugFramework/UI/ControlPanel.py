...
class EditExperimentWindow2(tk.Toplevel):
    def __init__(self, parent, data, update_callback):
        ...
        # Convert string booleans to actual booleans and strip whitespace
        self.input_vars = {key: tk.StringVar(value=str(value).strip()) for key, value in data.items()}
        self.entries = {}
        self.test_type_var = tk.StringVar(value=str(data.get('Test Type', TEST_TYPES[0])).strip())
        self.input_vars['Test Type'] = self.test_type_var

        # Correct boolean values
        self.bool_vars = {field: tk.StringVar(value='True' if str(data.get(field, '')).strip().upper() == 'TRUE' else 'False') for field in ['Reset', 'Reset on PASS', 'FastBoot', '600W Unit', 'Pseudo Config', 'Experiment']}
        self.input_vars.update(self.bool_vars)

        self.test_name_var = self.input_vars['Test Name']
        self.test_name_var.trace_add("write", lambda *args: update_callback(self.get_values()))

        self.create_form()
        self.update_display_fields()
        self.create_buttons()

        # Protocol for closing the window without calling the callback
    ...
    def create_form(self):
        ...
        for idx, (field, description) in enumerate(field_descriptions.items()):
            column = (idx % 2) * 2
            ...
            if column + 1 >= 3:  # En dos columnas
                row += 1

    def create_buttons(self):
        save_button = ttk.Button(self, text="Save", command=self.save_changes)
        save_button.grid(row=len(self.widgets)//2, column=0, columnspan=4, pady=10)  # Ajustar posición del botón
    ...
    def save_changes(self):
        ...
        if validation_errors:
            error_messages.append(f"Invalid entries in - {', '.join(validation_errors)}")
...
def TTL_testing(test_function, visual, cmds, chkcore, bucket, ttime, tnumber, exception_queue, cancel_flag):
	...
	try:
		...
		print(Fore.YELLOW + f"Framework Iteration Interrupted: {e}" + Fore.RESET)
		exception_queue.put(e)
	except TaskCancelledException as e:
		print(Fore.YELLOW + f"User Interruption in TTL_testing: {e}"+ Fore.RESET)
		exception_queue.put(e)
	except Exception as e:
		print(Fore.RED + f"Exception in TTL_testing: {e}"+ Fore.RESET)
		exception_queue.put(e)  # Put the exception into the queue
...
def Framework_call(Framework, data, S2TCONFIG, exception_queue, cancel_flag):
	try:
		...
		print(Fore.RED + f"Exception during Framework Execution: {e}" + Fore.RESET)
		exception_queue.put(e)  # Put the exception into the queue
...
def execute(task_name, target_func, exceptions, cancel_flag, args, use_process=True):
	...
	try:
		...
		handler.start()
		print(Fore.RED + f"Framework Control -- Starting new process task: {task_name} -- " +  Fore.RESET)
		...
		print(Fore.YELLOW + f"{task_name} Main Thread Execution Cancelled -- Exception {e}" +  Fore.RESET)
		exceptions.put(e)
		handler.terminate() if use_process else handler.stop()
		#handler.join()
		return False
		...
		print(Fore.RED + f"Exception occurred during {task_name}: {e}" +  Fore.RESET)
		#cancel_flag.set()
		handler.terminate() if use_process else handler.stop()
		handler.join()
		exceptions.put(e)
