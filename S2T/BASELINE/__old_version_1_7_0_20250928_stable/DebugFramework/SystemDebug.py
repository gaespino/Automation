...
class Framework:
	...
	def _handle_step_mode(self):
		...
		if self.execution_state.get_state('waiting_for_step'):
			...
			FrameworkUtils.FrameworkPrint("Step continue received - proceeding", 1)
...
class FrameworkExternalAPI:
	...
	def start_experiment_step_by_step(self, experiment_data: Dict, experiment_name: str = None, 
		...
		"""
		Start experiment in step-by-step mode for flow control.
		Returns immediately after starting, allowing flow to control execution.
		"""
		try:
			...
			def execute_experiment_thread():
				try:
					...
					print(f"Experiment execution error in thread: {e}")
			
			# Start experiment in background thread
			experiment_thread = threading.Thread(
				target=execute_experiment_thread,
				name=f"Experiment_{experiment_name or 'Unknown'}"
			)
			experiment_thread.daemon = True
			experiment_thread.start()
			
			# Give the experiment a moment to initialize
			import time
			time.sleep(1)
			
			# Verify the experiment started
			state = self.framework.get_execution_state()
