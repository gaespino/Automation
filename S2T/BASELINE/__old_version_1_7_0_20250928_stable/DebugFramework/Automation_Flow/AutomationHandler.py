...
class FlowProgressInterface:
	...
	def __init__(self, framework=None, utils=None, manager=None):
		...
		self.execution_manager.set_main_thread_handler
	...
	def setup_drawing_components(self):
		...
		#self.framework_integration = FrameworkIntegrationManager()
	...
	def _coordinate_progress_bar_styles(self, style_name, duration=None, bar_type='single'):
		...
		try:
			...
			if duration:
				self.root.after(duration, lambda: self.progress_bar.configure(style="TProgressbar"))
	...
	def set_framework_integration(self):
		...
		_manager = self.framework_manager
		_main_thread_handler = self.main_thread_handler
		_execution_state = self.execution_state

		# **ADD** Set main thread handler for execution manager
		self.execution_manager.main_thread_handler = _main_thread_handler
		self.execution_manager.framework_manager = _manager
		
		# **ADD** Set execution state for executor integration
		if hasattr(self, 'executor') and self.executor:
			self.executor.set_execution_state(execution_state)
		
		# Update framework integration manager
		self.framework_integration = FrameworkIntegrationManager(
			_manager, _main_thread_handler, _execution_state
		)
		
		# Create Framework API if manager is available
		if self.framework_manager:
			if self.framework_integration.initialize_framework():
				self.framework_api = self.framework_integration.framework_api
				self.execution_manager.framework_api = self.framework_api
	...
	def is_experiment_running(self, node):
		...
		if self.framework_integration:
			return self.framework_integration.is_experiment_running()
		return False
	...
	def cancel_execution(self):
		"""Cancel through API (matching ControlPanel)."""
	...
	def toggle_framework_hold(self):
		"""Toggle framework halt/continue functionality using command system (matching ControlPanel)."""
		if not self.framework_api:
			self.log_status("No Framework API available")
			return
	...
	def end_current_experiment(self):
		"""End experiment through API (matching ControlPanel)."""
		if not self.framework_api:
			self.log_status("No Framework API available")
			return
	...
	def process_updates(self):
		...
		# **ADD** Process MainThreadHandler updates (matching ControlPanel)
	...
	# **ADD** new method to handle MainThreadHandler updates:
...
def start_automation_flow_ui(framework=None, utils=None, manager=None):
	...
	# Set Framework integration if provided
	#if manager and framework:
	#	interface.set_framework_integration()
