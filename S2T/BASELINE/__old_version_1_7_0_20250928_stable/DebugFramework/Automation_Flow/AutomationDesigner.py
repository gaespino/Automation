...
class AutomationFlowDesigner:
    ...
    def draw_connection_ports(self, node_data: Dict):
        ...
        if node_type != 'EndNode':
            ...
            if node_type == 'StartNode':
                ports = [1]  # Start node only has success path
            else:
    ...
    def create_connection_dialog(self, from_node_id: str, to_node_id: str):
        ...
        port_var = tk.StringVar(value="1")  # Default to success path
        
        # Determine available ports based on source node type
        if from_node['type'] == 'StartNode':
            port_options = [("1 - Success Path", "1")]
        else:
            port_options = [
                ("0 - Failure Path", "0"),
                ("1 - Success Path", "1"),
                ("2 - Alternative Path", "2"),
                ("3 - Special Path", "3")
            ]
    ...
    def create_defeature_tab(self):
        ...
        for field_info in defeature_fields:
            ...
            if field_type == "entry":
                ...
                widget = ttk.Combobox(field_frame, textvariable=var, values=values, width=37)
