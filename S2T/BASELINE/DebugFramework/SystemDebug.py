...
class Framework:
	...
	def RecipeExecutor(self, data: Dict, S2T_BOOT_CONFIG: Dict = None,
		...
		if extmask:
			...
			FrameworkUtils.FrameworkPrint(f' Generating Core Disable external mask for cores: {config_updates["coredislist"]}')
			config_updates['extMask'] = gcm.build_disable_extmask(config_updates['coredislist'], 'core')
		elif config_updates.get('slicedislist'):
			FrameworkUtils.FrameworkPrint(f' Generating Slice Disable external mask for cores: {config_updates["slicedislist"]}')
			# Validate: in slice mode, the target core cannot be in the disable list
				...
				if target_mask is not None:
					try:
						if int(target_mask) in config_updates['slicedislist']:
							raise ValueError(
								f"Configuration Error: target core {target_mask} is in the Slice Disable List "
								f"{config_updates['slicedislist']}. A core cannot be both the test target and disabled."
							)
