...
class StatusExecutionPanel:
    ...
    def create_experiment_stats_section(self):
        ...
        self.exp_iteration_label.pack(side=tk.LEFT, padx=(10, 0))
        
        self.exp_pass_rate_label = ttk.Label(exp_stats_frame, text="Pass Rate: 0%")
        self.exp_pass_rate_label.pack(side=tk.LEFT, padx=(20, 0))
       
        self.exp_fail_rate_label = ttk.Label(exp_stats_frame, text="Fail Rate: 0%")
        self.exp_fail_rate_label.pack(side=tk.LEFT, padx=(30, 0))
    ...
    def update_experiment_stats(self, iteration: int = None, total_iterations: int = None,
                               pass_rate: float = None, fail_rate: float = None, recommendation: str = None):
        ...
        if pass_rate is not None:
            ...
            color = "green" if pass_rate >= 80 else "orange" if pass_rate >= 50 else "red"
            self.exp_pass_rate_label.configure(
        ...
        if fail_rate is not None:
            # Color code the pass rate
            color = "red" if fail_rate >= 80 else "orange" if fail_rate >= 50 else "green"
            self.exp_fail_rate_label.configure(
        ...
        if recommendation is not None:
            ...
            rec_color = "green" if "excellent" in recommendation.lower() or "good" in recommendation.lower() else \
                       "red" if "critical" in recommendation.lower() or "poor" in recommendation.lower() else \
                       "orange" if "marginal" in recommendation.lower() else "black"
            
            self.exp_recommendation_label.configure(
