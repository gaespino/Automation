			...
			for line in lines:
				for search_string in FailString:
					if (search_string in line) if casesens else (search_string.lower() in line.lower()):
						...
						obj_file = obj_file.replace('.obj','') + '_FAIL' if obj_file != None else 'NA'
						return obj_file
			...
			if not obj_file:
				for line in reversed(lines):
					...
					for search_string in HangString:
						if (search_string in line) if casesens else (search_string.lower() in line.lower()):
							obj_file = extract_obj_from_line(line)
							obj_file = obj_file.replace('.obj','') + '_HANG' if obj_file != None else 'NA'
							return obj_file
...
class FrameworkConfigConverter:
	...
	def _get_field_to_ini_mapping(self):
		"""
		...
		return {
			'dragon': {
				# DRG_INIT section
				'dragon_startup_cmd': ('DRG_INIT', 'STARTUP_EFI'),
	...
	def _dict_to_config_objects(self, config_dict):
	...
	def _update_config_section(self, config_obj, config_type):
			...
			for field_name in dir(config_obj):
				...
				if isinstance(mapping, dict):
					if current_flow_type in mapping:
						section, key = mapping[current_flow_type]
					else:
	...
	def _get_slice_flow_definition(self):
		...
		return [
			...
			("VVAR_EXTRA", "VVAR_SETUP", "VVAR_EXTRA", " "),
			("SLICE_CONTENT", "SETUP_SLICE", "SLICE_CONTENT", "FS1:\\content\\Dragon\\GNR1C_Q_Slice_2M_pseudoSBFT_System"),
