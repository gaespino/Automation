...
class EditExperimentWindow(tk.Toplevel):
	...
	def create_new_experiment(self):
		...
		try:
			...
			if template_choice == "empty":
				...
				new_experiment_data = self.create_from_current_values()
			else:
				# Use selected experiment as template
			...
			messagebox.showerror("Error", f"Failed to create new experiment: {str(e)}")

	def save_as_new_experiment(self):
		"""Save current configuration as a new experiment (for edit mode)"""
		try:
			# Get new experiment name
			new_name = self.get_new_experiment_name()
			if not new_name:
			...
			if self.add_new_experiment_callback:
				self.add_new_experiment_callback(new_name, new_experiment_data)
				messagebox.showinfo("Success", f"New experiment '{new_name}' created successfully!")
				# Don't close the window in edit mode, user might want to continue editing
			else:
				messagebox.showwarning("Error", "Cannot add new experiment - callback not available")
	...
	def create_field_widget(self, parent, field, row, tab_name):
		"""Create widget for a specific field with optimized widths"""
		...
		# Create label with mandatory indicator - reduced width
		label_text = field
		...
		# Add validation indicator - smaller
		validation_indicator = tk.Label(parent, text="", width=1,
									  bg="SystemButtonFace", relief="flat")
		...
		if bool in field_types:
			...
			# File/folder selection fields - optimized layout
	...
	def validate_field(self, field):
		"""Validate a single field and update its indicator"""
		...
		if self._is_field_disabled(field, content_type):
			indicator.configure(text="", bg="SystemButtonFace")
			return True
		...
		try:
			...
			# Type-specific validation
				...
				if field in ['Start', 'End', 'Steps']:
					...
					try:
						int_val = int(value)
						if field in ['COM Port'] and not (0 <= int_val <= 256):
							is_valid = False
				...
				# SIMPLIFIED: Path validation - just check for valid string format
				if field in ['TTL Folder', 'Scripts File', 'Post Process', 'ShmooFile'] and value:
					is_valid, error_message = self._validate_path_format(field, value)
				
				# Remote path validation - just check format, not existence
				if field in remote_path_fields and value:
		...
		# Update indicator with safe background handling
		try:
			if is_valid:
				indicator.configure(text="✓", fg="green", bg="SystemButtonFace")
			else:
				indicator.configure(text="✗", fg="red", bg="SystemButtonFace")
		except tk.TclError:
			# Fallback if bg option is not supported
			if is_valid:
	...
	def _validate_numeric_field_by_type(self, field, value, type_value):
		"""Validate Start, End, Steps fields based on the Type field value"""
		is_valid = True
		error_message = ""
		
		try:
			if type_value.lower() == 'frequency':
				# For Frequency type, Start, End, Steps should be integers
				try:
					int_val = int(value)
					...
					is_valid = False
					error_message = f"{field} must be integer for Frequency type"
					
			elif type_value.lower() == 'voltage':
				# For Voltage type, Start, End, Steps can be floats
				try:
					float_val = float(value)
					if field == 'Steps' and float_val <= 0:
						is_valid = False
						error_message = "Steps must be positive number for Voltage"
					# You can add more voltage-specific validation here if needed
					# For example, voltage range limits:
					# elif field in ['Start', 'End'] and not (0.0 <= float_val <= 5.0):
					#     is_valid = False
					#     error_message = f"{field} must be between 0.0 and 5.0V"
				except ValueError:
					is_valid = False
					error_message = f"{field} must be a number for Voltage type"
					
			else:
				# For other types or when Type is not set, default to float validation
				try:
					float_val = float(value)
					if field == 'Steps' and float_val <= 0:
						is_valid = False
	...
	def validate_all_fields(self):
		...
		# Update status
		if error_count == 0:
			self.status_label.configure(text="All fields valid", fg="green")
			self.validation_count_label.configure(text="")
		else:
			self.status_label.configure(text="Validation errors found", fg="red")
			self.validation_count_label.configure(text=f"{error_count} errors")
	...
	def save_changes(self):
		"""Save changes with comprehensive validation (for edit mode)"""
		# Validate all fields first
		self.validate_all_fields()
		
		# Check for validation errors
		error_count = 0
		for field in self.validation_indicators:
			indicator = self.validation_indicators[field]
			if indicator.cget('text') == '✗':
				error_count += 1
		...
		if conversion_errors:
			messagebox.showwarning("Conversion Warnings", 
								 f"Some fields could not be converted:\n" + "\n".join(conversion_errors[:5]))
...
class DebugFrameworkControlPanel:
	...
	def _execute_framework_test(self, exp_data, s2t_config, experiment_name, framework_instance_id):
		...
		try:
			...
			external_masking = extmask if extmask not in [None, 'Default'] else None
		   # Execute through API
