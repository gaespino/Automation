...
class ExecutionStateMachine:
	...
	def get_current_statistics(self) -> Dict[str, Any]:
		...
		return {
			...
			'recent_trend': self._analyze_recent_trend(results),
			'recommendation': self._get_recommendation(pass_rate, total_completed),
			'sufficient_data': self._has_sufficient_data(pass_rate, total_completed),
			'current_iteration': self.current_experiment_stats['current_iteration'],
			'total_iterations': self.current_experiment_stats['total_iterations'],
			'experiment_name': self.current_experiment_stats['experiment_name'],
	...
	def _analyze_recent_trend(self, results: List[Dict]) -> str:
		...
		if fails == total:
			return "repro"
		elif fails < total and fails >= (total/2):
			return "flaky"
		elif fails > 1:
			return "unstable"
		elif fails == 0:
	...
	def _get_recommendation(self, pass_rate: float, total: int) -> str:
		"""Get recommendation based on current statistics"""
		if total < 3:
			return "continue"
		elif self._has_sufficient_data(pass_rate, total):
			if pass_rate >= 90:
	...
	def _has_sufficient_data(self, pass_rate: float, total: int) -> bool:
...
class MainThreadHandler(IStatusReporter):
	...
	#def _handle_strategy_progress_safe(self, progress_data):
	#	"""Handle strategy progress updates safely"""
	#	try:
	#		# Mark strategy progress as active
	#		self.ui._strategy_progress_active = True
	#		self.show_debug_messages(f"BEFORE Strategy Progress Update:")
	#
	#			# Use coordinated progress update
	#		self.ui._coordinate_progress_updates('strategy_progress', progress_data)
	#		
	#		# Schedule to clear strategy progress flag after delay
	#		self.root.after(1000, lambda: setattr(self.ui, '_strategy_progress_active', False))
	#		self.show_debug_messages(f"AFTER Strategy Progress Update:")
	#		
	#	except Exception as e:
	#		print(f"Strategy progress update error: {e}")

	#def _handle_experiment_info_safe(self, exp_info):
	#	"""Handle experiment info updates safely"""
	#	try:
	#		self.ui.current_experiment_name = exp_info['name']
	#		self.ui.total_iterations_in_experiment = exp_info['total_iterations']
	#		
	...
	#		)
	#	except Exception as e:
	#		print(f"Experiment info update error: {e}")

	#def _handle_iteration_info_safe(self, iter_info):
	#	"""Handle iteration info updates safely"""
	#	try:
	...
	#		
	#		# Only force overall progress if no strategy progress is active
