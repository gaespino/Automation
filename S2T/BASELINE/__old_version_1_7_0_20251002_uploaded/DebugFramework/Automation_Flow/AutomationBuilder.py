...
class FlowExecutionManager:
	...
	def __init__(self, flow_config, flow_execution_state, framework_manager):
		...
		# Single shared Framework API (created on start, destroyed on end)
	...
	def start_execution(self, executor):
		"""Start flow execution with single shared Framework API."""
		...
		# STEP 1: Create single Framework API for entire flow execution
		success, api_or_error = self._create_shared_framework_api()
		if not success:
			return False, f"Failed to create Framework API: {api_or_error}"

		# STEP 2: Share API with all components
		self._distribute_framework_api()

		# STEP 3: Start execution thread
		self.executor = executor
		self.flow_execution_state.reset_for_new_execution()
		self.flow_execution_state.is_running = True
		self.thread_active = True

		# Create primitive data for thread
		flow_data = self._create_primitive_flow_data()

		# Start execution thread
		self.execution_thread = threading.Thread(
			target=self._execute_flow_thread,
			args=(flow_data, self.shared_framework_api, self.framework_instance_id),
			daemon=True,
			name=f"FlowExecution_{self.framework_instance_id}"
		)
		self.execution_thread.start()
		
		return True, "Execution started with shared Framework API"

	def _create_shared_framework_api(self):
		"""Create single Framework API for entire flow execution."""
		try:
	...
	def _distribute_framework_api(self):
		"""Distribute shared Framework API to all components that need it."""
		try:
			# Set on executor
			if self.executor:
				self.executor.set_framework_api(self.shared_framework_api)
				print(f"Shared Framework API with executor")

			# Set on flow config
			if self.flow_config:
				self.flow_config.framework_api = self.shared_framework_api

			# Set on main interface (for button controls)
			if hasattr(self, 'main_interface'):
				self.main_interface.framework_api = self.shared_framework_api
				if hasattr(self.main_interface, 'framework_integration'):
					self.main_interface.framework_integration.framework_api = self.shared_framework_api

		except Exception as e:
			print(f"Error distributing Framework API: {e}")

	def start_execution_old(self, executor):
	...
	def cancel_execution(self):
		"""Cancel current execution using shared Framework API directly."""
		if not self.shared_framework_api:
			return False, "No shared Framework API available"

		try:
			result = self.shared_framework_api.cancel_experiment()
			
			# Update local state
			self.flow_execution_state.cancel("User requested cancellation")
	...
	def end_current_experiment(self):
		"""End current experiment using shared Framework API directly."""
		if not self.shared_framework_api:
			return False, "No shared Framework API available"
		
		try:
			result = self.shared_framework_api.end_experiment()
			return result['success'], result['message']
		except Exception as e:
			return False, f"End experiment failed: {str(e)}"

	def cancel_execution_old(self):
	...
	def _execute_flow_thread(self, flow_data, framework_api, framework_instance_id):
		...
		try:
			# FIXED: Compare with the correct variable
			if framework_instance_id != self.framework_instance_id:
				if self.main_thread_handler:
					self.main_thread_handler.queue_status_update({
						'type': 'flow_execution_error',
						'data': "Framework instance mismatch - aborting thread"
					})
				return
			...
			if self.main_thread_handler:
				self.main_thread_handler.queue_status_update({
					...
					'data': 'Flow execution was cancelled'
				})
		except Exception as e:
			if self.main_thread_handler:
				self.main_thread_handler.queue_status_update({
	...
	def _execute_flow_thread_old(self, flow_data, framework_api, framework_instance_id):
		"""Execute flow in separate thread with proper command checking."""
		try:
		...
		while (current_node is not None and 
			...
			if framework_instance_id != self.framework_instance_id:
				if self.main_thread_handler:
					self.main_thread_handler.queue_status_update({
						'type': 'flow_execution_error',
						'data': "Framework instance changed during execution"
					})
				break
	...
	def _execute_single_node_monitored(self, node, framework_instance_id):
		...
		try:
			if self.shared_framework_api:
				node.framework_api = self.shared_framework_api
				print(f"Set framework_api on node {node.ID}: {self.shared_framework_api}")
			else:
				print(f"WARNING: No framework_api available for node {node.ID}")
				
			# FIXED: Framework instance verification
			if framework_instance_id and framework_instance_id != self.framework_instance_id:
				return False
			...
			# Check for system-level failures that should cancel entire flow
	...
	def _prepare_framework_for_node(self, node):
		"""Prepare shared Framework API for new node execution."""
		try:
			if self.shared_framework_api:
				# Clean up any previous experiment state
				self.shared_framework_api.cleanup_step_experiment()
				
				# Small delay to ensure cleanup
				time.sleep(0.1)
				
				print(f"Framework prepared for node: {node.ID}")
		except Exception as e:
			print(f"Error preparing Framework for node {node.ID}: {e}")
						
	def _execute_single_node_monitored_old(self, node, framework_instance_id):
		"""Execute a single node with Framework API integration and monitoring."""
		try:
	...
	def cleanup_shared_framework_api(self):
		"""Clean up shared Framework API at end of execution."""
		try:
			if self.shared_framework_api:
				print(f"Cleaning up shared Framework API: {self.framework_instance_id}")
				
				# Cancel any running experiments
				try:
					self.shared_framework_api.cancel_experiment()
					time.sleep(0.1)
				except:
					pass
				...
				print("Shared Framework API cleanup completed")

		except Exception as e:
			print(f"Error cleaning up shared Framework API: {e}")

	def _cleanup_execution_thread(self, flow_data, framework_instance_id):
		"""Cleanup execution thread and shared Framework API."""
		try:
			self.thread_active = False
			self.flow_execution_state.finalize_execution("flow_execution_complete")
			
			# Clean up shared Framework API
			self.cleanup_shared_framework_api()
			...
			print(f"Execution thread cleanup error: {e}")
						
	def _cleanup_execution_thread_old(self, flow_data, framework_instance_id):
