...
class StepExperimentContext:
	"""Context manager for step-by-step experiments with guaranteed cleanup."""
	
	def __init__(self, framework_api, experiment_data, experiment_name=None, **kwargs):
	...
	def __enter__(self):
		try:
			print(f"Context manager starting experiment: {self.experiment_name}")
			
			self.start_result = self.framework_api.start_experiment_step_by_step(
			...
			if self.start_result['success']:
				...
				print(f"Context manager: experiment started successfully")
			...
			self.experiment_started = False
			print(f"Context manager: experiment startup failed: {e}")
			raise

	def __exit__(self, exc_type, exc_val, exc_tb):
		if self.experiment_started:
			try:
				print(f"Context manager cleaning up experiment: {self.experiment_name}")
				self.framework_api.cleanup_step_experiment()
				print(f"Context manager cleanup completed: {self.experiment_name}")
			except Exception as e:
				print(f"Error during context cleanup for {self.experiment_name}: {e}")
		else:
			print(f"Context manager: no cleanup needed for {self.experiment_name}")
...
class FlowInstance(ABC):
	...
	def run_experiment_old(self):
		"""Main experiment runner with context manager integration."""
		self.logger(f"Running Experiment: {self.Name} (ID: {self.ID})")
		
		if self.framework_api is not None:
			try:
				self._run_framework_experiment_with_context()
			except Exception as e:
				self.logger(f"Framework experiment failed: {e}")
				self.runStatusHistory = ['FAILED']
			...
			self.runStatusHistory = ['PASS']  # or your test mode logic
	...
	def _run_framework_experiment_with_context(self):
		"""Run experiment using context manager with proper lifecycle management."""
		experiment_name = f"{self.Name}_{self.ID}"
		
		try:
			with StepExperimentContext(
				...
				# Monitor the experiment - this should handle the complete lifecycle
				self._monitor_complete_experiment_lifecycle()
	...
	# OLD -- Not using context manager	
	def _run_framework_experiment(self):
		"""
		Run experiment with comprehensive cleanup handling.
		"""
		...
		try:
			...
			self.logger(f"Starting step-by-step experiment: {experiment_name}")
			...
			self.logger(f"Step-by-step experiment started successfully")
			self.logger(f"Thread ID: {result.get('thread_id')}")
			self.logger(f"Thread Name: {result.get('thread_name')}")
			
			# Monitor experiment execution using event-driven approach
			self._monitor_step_by_step_execution_with_events()
			
		except KeyboardInterrupt:
			self.logger(f"Experiment interrupted by user")
			self.runStatusHistory = ['CANCELLED']
		except Exception as e:
			self.logger(f"Error running Framework experiment: {e}")
			self.runStatusHistory = ['FAILED']
		finally:
			# Always ensure cleanup if experiment was started
			if experiment_started and self.framework_api:
				self.logger(f"Performing experiment cleanup...")
				try:
					self.framework_api.cleanup_step_experiment()
					self.logger(f"Experiment cleanup completed")
				except Exception as cleanup_error:
					self.logger(f"Error during experiment cleanup: {cleanup_error}")

	def _monitor_step_by_step_execution_with_events(self):
		"""
		Monitor with TestStatus-aware hardware failure detection.
		"""
		iteration_count = 0
		max_iterations = self._get_max_iterations()
		collected_results = []
		monitoring_active = True
		hardware_failure_detected = False
		
		self.logger(f"Starting event-driven step monitoring. Max iterations: {max_iterations}")
		
		try:
			while monitoring_active and not hardware_failure_detected:
				# Wait for next event from the experiment thread
				event_result = self.framework_api.wait_for_next_iteration_event(timeout=120)
				
				if not event_result['success']:
					if event_result.get('timeout'):
						self.logger(f"Timeout waiting for iteration event - possible hardware issue")
						hardware_failure_detected = True
					elif event_result.get('cleanup_requested'):
						self.logger(f"Experiment cleanup was requested - stopping monitoring")
						monitoring_active = False
					elif event_result.get('thread_died'):
						self.logger(f"Experiment thread died unexpectedly - hardware issue suspected")
						hardware_failure_detected = True
					else:
						self.logger(f"Error waiting for iteration event: {event_result.get('error')}")
				...
				if event_type == StatusEventType.ITERATION_COMPLETE.value:
					...
					self.logger(f"  Status: {status}")
					self.logger(f"  Classification: {status_classification.get('category', 'unknown')}")
					self.logger(f"  Scratchpad: {scratchpad}")
					self.logger(f"  Pass rate: {statistics.get('pass_rate', 0)}%")
					...
					if status_classification.get('is_valid_test', False):
						...
						if decision == 'continue':
							...
							if not continue_result['success']:
								...
								self.logger(f"Continue command sent successfully")
								
						elif decision == 'end':
							end_result = self.framework_api.end_current_experiment()
							if end_result['success']:
								self.logger(f"End command sent successfully")
							else:
								self.logger(f"Failed to send end command: {end_result['message']}")
							# Continue monitoring for experiment_complete event
						...
						# For non-test statuses, just continue
						self.logger(f"Non-test status received, continuing monitoring")
					...
					self.logger(f"Unknown event type: {event_type}")
		
		except KeyboardInterrupt:
			self.logger(f"Monitoring interrupted by user")
			monitoring_active = False
		except Exception as e:
			self.logger(f"Error in step monitoring: {e}")
			hardware_failure_detected = True
			monitoring_active = False
		finally:
			# Handle final results based on how monitoring ended
			if hardware_failure_detected:
				self.logger(f"Hardware failure detected - setting FAILED status")
				self.runStatusHistory = [TestStatus.FAILED.value]  # This will trigger flow abortion
			elif not monitoring_active and not collected_results:
				self.logger(f"Monitoring stopped with no results")
				self.runStatusHistory = [TestStatus.FAILED.value]
			elif not hasattr(self, 'runStatusHistory') or not self.runStatusHistory:
				# Process any collected results
				if collected_results:
					self._process_final_results_from_events(collected_results, [])
				else:
					self.runStatusHistory = [TestStatus.FAILED.value]
			
			self.logger(f"Final status history: {len(self.runStatusHistory)} results")

	# OLD -- Check and delete	
...
class FlowTestExecutor:
	...
	def execute(self):
		"""
		Execute the flow with comprehensive termination handling.
		"""
		...
		try:
			...
			while current_node is not None and node_count < 50:
				# ✅ Check for any termination conditions FIRST
				...
				if self.framework_api:
					...
					# Track nodes with potential active experiments
					with self._cleanup_lock:
						if current_node not in self._nodes_with_active_experiments:
							self._nodes_with_active_experiments.append(current_node)
				...
				try:
					...
					# Always cleanup the current node
					self._cleanup_node_experiment(current_node)
	...
	def _trigger_hardware_failure_termination(self, failed_node, reason=None):
		"""
		✅ NEW: Trigger immediate hardware failure termination.
		
		Args:
			failed_node: Node that detected the hardware failure
	...
	def _should_terminate_flow(self) -> bool:
		"""
		✅ ENHANCED: Check if flow should terminate for any reason.
		"""
	...
	def _check_node_termination_request(self, node) -> bool:
		"""
		✅ NEW: Check if node requested flow termination due to unwired port.
		
		Args:
			node: Node to check for termination request
			
		Returns:
			bool: True if termination was requested
		"""
...
class FlowTestBuilder:
	...
	def build_flow(self, rootID):
		...
		✅ ENHANCED: Pass built nodes reference to executor for end node finding.
		"""
		try:
