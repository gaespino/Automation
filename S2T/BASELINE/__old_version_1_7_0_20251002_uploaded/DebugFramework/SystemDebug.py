...
class TestExecutor:
	...
	def _check_cancellation(self):
		...
		if (hasattr(self.config, 'execution_cancelled') and 
			...
			command_result != "CONTINUE"):  # ✅ Allow retries to continue
	...
	def _execute_custom_script(self, script, post = False):
		...
		if post:
			...
			if self.config.content == ContentType.BOOTBREAKS:
				self.gdflog.log(f"Executing Custom script at reached boot Breakpoint: {script}", 1)
			elif self.config.content == ContentType.PYSVCONSOLE:
				self.gdflog.log(f"Executing Custom script before test: {script}", 1)
	...
	def execute(self, executor: TestExecutor, halt_controller = None) -> List[TestResult]:
		...
		try:
			for i in range(self.loops):
				...
				# ✅ Log state before each iteration
			...
			# ✅ Log state after each iteration
				...
				if halt_controller.execution_state.is_step_mode_enabled():
					...
					if command_result == "CONTINUE" and halt_controller.execution_state.has_command(ExecutionCommand.STEP_CONTINUE):
						...
						executor.gdflog.log(f"Loops Received CANCEL command in step mdoe (wait) - stopping strategy", 2)
						results.append(TestResult(status="CANCELLED", name=executor.config.name, iteration=i + 1))
						break
				else:
...
class Framework:
	...
	def _handle_step_mode(self):
		...
		if self.execution_state.get_state('waiting_for_step'):
			...
			while True:
				...
				# ✅ Progressive sleep with periodic logging
	...
	def _wait_for_step_command(self, current_iteration: int, total_iterations: int, 
		...
		log_func = logger.log if logger else FrameworkUtils.FrameworkPrint
	...
	def acknowledge_cancel_command(self, reason="Execution fully cancelled"):
		...
		if self.execution_state.has_command(ExecutionCommand.CANCEL):
			...
			FrameworkUtils.FrameworkPrint(f"CANCEL command acknowledged: {reason}", 1)
			return True
		return False
	...
	def acknowledge_end_command(self, reason="Experiment ended after iteration completion"):
		...
		if self.execution_state.has_command(ExecutionCommand.END_EXPERIMENT):
			...
			FrameworkUtils.FrameworkPrint(f"END command acknowledged: {reason}", 1)
			return True
		return False
	...
	def is_waiting_for_step_command(self) -> bool:
		...
		FrameworkUtils.FrameworkPrint(f"Step Mode: {self.execution_state.is_step_mode_enabled()}:Step Status: {self.execution_state.get_state('waiting_for_step')}")
		return (self.execution_state.is_step_mode_enabled() and 
				self.execution_state.get_state('waiting_for_step'))

	# ==================== EXECUTION METHODS ======================

	def _create_executor(self) -> TestExecutor:
		# ✅ Log state before creating executor
		...
		# ✅ Log state after creating executor
...
class FrameworkExternalAPI:
	...
	def start_experiment_step_by_step(self, experiment_data: Dict, experiment_name: str = None, 
		...
		with self._step_thread_lock:
			try:
				...
				self._step_iteration_queue = queue.Queue(maxsize=100)
	...
	def wait_for_next_iteration_event(self, timeout: int = 60) -> Dict:
		...
		try:
			...
			# Wait for next event
			event_data = self._step_iteration_queue.get(timeout=timeout)
			...
			if event_data['event_type'] == StatusEventType.THREAD_COMPLETE.value:
				...
				return self.wait_for_next_iteration_event(timeout)
			...
			if hasattr(self.framework.status_manager, 'send_update'):
				...
				def intercepted_send_update(event_type, **kwargs):
					...
					try:
						...
						# ✅ Capture iteration completion with statistics from status handler
						if event_name == 'ITERATION_COMPLETE':
							# Get current statistics from the status handler
							current_stats = {}
							try:
								if hasattr(self.framework.status_manager, '_status_reporter'):
									state_machine = self.framework.status_manager._status_reporter.state_machine
							...
							iteration_event = {
								...
								'success': True,
								'iteration': kwargs.get('current_iteration', 0),
								...
								'timestamp': time.time(),
								'statistics': current_stats,  # ✅ Include updated statistics from status handler
								'raw_kwargs': kwargs
							}
	...
	def end_current_experiment(self) -> Dict:
		"""Alias for end_experiment for better API consistency in flows."""
	...
	def get_iteration_statistics(self) -> Dict:
		...
		try:
			...
			if (hasattr(self.framework, 'status_manager') and 
				hasattr(self.framework.status_manager, '_status_reporter') and
				hasattr(self.framework.status_manager._status_reporter, 'state_machine')):
	...
	def _analyze_recent_trend(self, recent_results: List) -> str:
		...
		passes = sum(1 for r in recent_3 if r.get('status') in ['PASS', '*', 'SUCCESS'])
