...
class ExecutionStateMachine:
	...
	def _handle_experiment_start(self, data) -> Dict[str, Any]:
		...
		return {
			'log_message': f"🚀 Started Experiment: {data['experiment_name']}",
			'log_message_2': f"📋 Strategy: {data['strategy_type']} with {data['total_iterations']} iterations",
	...
	def _handle_iteration_start(self, data) -> Dict[str, Any]:
		...
		return {
			'log_message': f"▶️ Starting Iteration {data['iteration']}: {data['status']}",
			'iteration_info': {
	...
	def _handle_iteration_progress(self, data) -> Dict[str, Any]:
		...
		if self._is_boot_phase(status):
			...
			return {
				'log_message': f"⚙️ Iteration {data['iteration']}: {status}",
				'iteration_info': {
					'current': data['iteration'],
					'progress_weight': data.get('progress_weight', 0.0)
				},
	...
	def _handle_iteration_cancelled(self, data) -> Dict[str, Any]:
		...
		return {
			'log_message': f"🚫 CANCELLED Iteration {data['iteration']}: {data['status']}",
			'iteration_info': {
				'current': data['iteration'],
				'progress_weight': 1.0
			},
	...
	def _handle_strategy_complete(self, data) -> Dict[str, Any]:
		...
		log_messages = [
			"=" * 50,
			f"🏁 STRATEGY COMPLETE: {data.get('test_name', 'Unknown Test')}",
			"=" * 50,
			f"📋 Strategy Type: {strategy_type}",
			f"📊 Total Tests: {data.get('total_tests', 0)}",
		...
		if strategy_type == 'Shmoo':
			if data.get('shmoo_dimensions'):
				log_messages.append(f"📐 Shmoo Dimensions: {data['shmoo_dimensions']}")
			if data.get('x_axis_config'):
				x_config = data['x_axis_config']
				log_messages.append(f"📈 X-Axis: {x_config.get('Type', 'Unknown')} - {x_config.get('Domain', 'Unknown')}")
			if data.get('y_axis_config'):
				y_config = data['y_axis_config']
				log_messages.append(f"📈 Y-Axis: {y_config.get('Type', 'Unknown')} - {y_config.get('Domain', 'Unknown')}")
		
		# Add status breakdown
		if 'status_counts' in data and data['status_counts']:
			log_messages.append("📈 Results Summary:")
		...
		if data.get('failure_patterns'):
			log_messages.append("🔍 Top Failure Patterns:")
	...
	def _handle_experiment_complete(self, data) -> Dict[str, Any]:
		...
		return {
			'log_message': f"🎉 EXPERIMENT COMPLETED: '{data['test_name']}'",
	...
	def _handle_experiment_failed(self, data) -> Dict[str, Any]:
		...
		return {
			'log_message': f"💥 EXPERIMENT FAILED: '{data['experiment_name']}' - {data['reason']}",
	...
	def _handle_experiment_ended_by_command(self, data) -> Dict[str, Any]:
		...
		return {
			'log_message': f"🛑 Experiment ended by {reason} after {completed}/{total} iterations"
		}
	...
	def _handle_execution_halted(self, data) -> Dict[str, Any]:
		...
		return {
			'log_message': f"⏸️ Execution HALTED at iteration {iteration} of {test_name}",
			'status_label': {'text': ' Halted ', 'bg': 'orange', 'fg': 'black'},
			'status_update': "HALTED - Waiting for Continue",
			'button_update': {'hold_button': {'text': 'Continue', 'style': 'Continue.TButton'}}
		}
	...
	def _handle_execution_resumed(self, data) -> Dict[str, Any]:
		...
		return {
			'log_message': f"▶️ Execution RESUMED at iteration {iteration} of {test_name}",
			'status_label': {'text': ' Running ', 'bg': '#BF0000', 'fg': 'white'},
			'status_update': "Resumed",
			'button_update': {'hold_button': {'text': 'Hold', 'style': 'Hold.TButton'}}
		}
	...
	def _handle_execution_cancelled(self, data) -> Dict[str, Any]:
		...
		if 'completed_experiments' in data and 'total_experiments' in data:
			...
			log_messages.append(f"📊 Progress: {completed}/{total} experiments completed before cancellation")
		
		# Add current experiment info if available
		if experiment_name != 'Unknown':
			log_messages.append(f"🔄 Last experiment: {experiment_name}")
	...
	def _handle_execution_setup(self, data) -> Dict[str, Any]:
		...
		return {
			'log_message': f"🚀 Setup: {total_experiments} experiments queued",
			'log_message_2': f"📋 Experiments: {', '.join(experiment_names[:3])}{'...' if len(experiment_names) > 3 else ''}",
			'execution_setup': {
	...
	def _handle_execution_prepared(self, data) -> Dict[str, Any]:
		...
		return {
			'log_message': "🔧 Framework prepared for execution",
			'status_update': "Ready"
		}
	...
	def _handle_execution_finalized(self, data) -> Dict[str, Any]:
		...
		return {
			'log_message': f"🏁 Execution finalized: {reason}",
	...
	def _handle_step_mode_enabled(self, data) -> Dict[str, Any]:
		...
		return {
			'log_message': "🔄 Step-by-step execution mode enabled",
	...
	def _handle_step_mode_disabled(self, data) -> Dict[str, Any]:
		...
		return {
			'log_message': "▶️ Step-by-step mode disabled - continuous execution",
	...
	def _handle_step_iteration_complete(self, data) -> Dict[str, Any]:
		...
		log_messages = [
			f"⏸️ STEP MODE: Iteration {iteration}/{total} COMPLETE",
			f"📊 Current Stats - Pass: {stats.get('pass_count', 0)}, Fail: {stats.get('fail_count', 0)}",
			"⏳ Waiting for command..."
		]
	...
	def _handle_step_continue_issued(self, data) -> Dict[str, Any]:
		...
		return {
			'log_message': f"▶️ CONTINUE command - proceeding to iteration {iteration + 1}/{total}",
			'status_update': "Step: Continuing",
	...
	def _handle_experiment_index_update(self, data) -> Dict[str, Any]:
		...
		return {
			'log_message': f"📊 Starting Experiment {current_index + 1}/{total_experiments}: {experiment_name}",
	...
	def _handle_all_experiments_complete(self, data) -> Dict[str, Any]:
		...
		self.current_state = ExecutionState.COMPLETED
		return {
			'log_message': f"🎉 All {data['total_executed']} experiments completed",
			'status_label': {'text': ' Completed ', 'bg': '#006400', 'fg': 'white'},
			'enable_buttons': True,
	...
	def _handle_execution_ended(self, data) -> Dict[str, Any]:
		...
		if 'completed_experiments' in data and 'total_experiments' in data:
			completed = data['completed_experiments']
			total = data['total_experiments']
			log_messages.append(f"📊 Progress: {completed}/{total} experiments completed before ending")
	...
	def _handle_boot_phase_progress(self, data) -> Dict[str, Any]:
		...
		return {
			'log_message': f"🔄 Boot Progress [{progress_percent}%]: {boot_stage}",
			'iteration_info': {
				'current': iteration,
	...
	def _handle_test_content_progress(self, data) -> Dict[str, Any]:
		...
		return {
			'log_message': f"🧪 Test Content [{progress_percent}%]: {content_stage}",
			'iteration_info': {
				'current': iteration,
				'progress_weight': progress_weight
			},
	...
	def _apply_ui_updates(self, updates: Dict[str, Any]):
		...
		try:
			...
			if 'strategy_progress' in updates:
				...
				# DEBUG: Before update
				print(f"BEFORE Strategy Progress Update:")
				...
				# DEBUG: After update
				print(f"AFTER Strategy Progress Update:")
				self.ui.debug_progress_state()
			...
			if 'iteration_info' in updates:
				...
				print(f"BEFORE Iteration Info Update:")
				self.ui.debug_progress_state()
				...
				print(f"AFTER Iteration Info Update:")
				self.ui.debug_progress_state()
			
			# Handle progress updates
			if 'update_progress' in updates:
				print(f"BEFORE Overall Progress Update:")
				self.ui.debug_progress_state()
				
				self.ui._update_overall_progress()
				
				print(f"AFTER Overall Progress Update:")
				self.ui.debug_progress_state()
