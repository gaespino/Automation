...
class DebugFrameworkControlPanel:
	...
	def __init__(self, root, Framework):
		...
		if Framework:
			...
			self.execution_state = self.Framework.execution_state#execution_state
		else:
			self.Framework = None
	...
	'''	def cancel_tests(self):
			...
			try:
				...
				# CRITICAL: Schedule proper cleanup in main thread
				self.root.after(1000, self._cleanup_after_cancel)
				
			except Exception as e:
				self.log_status(f"Error during cancellation: {e}")'''
	'''def _cleanup_after_cancel(self):
			"""Cleanup after cancellation - runs in main thread"""
			try:
				# Wait for thread to finish
				if self.framework_thread and self.framework_thread.is_alive():
					# Give thread more time to finish
					self.root.after(2000, self._force_cleanup_after_cancel)
					return
				
				# Thread finished, safe to cleanup
				self._complete_cancel_cleanup()
				
			except Exception as e:
				self.log_status(f"Error in cleanup after cancel: {e}")'''
	...
	'''	def start_tests_thread(self):
			"""FIXED: Start tests with proper thread isolation"""
			if self.thread_active:
				self.log_status("Tests already running")
				return
			
			# CRITICAL: Prepare primitive data for thread (NO Tkinter variables)
			enabled_experiments = []
			
			# Use experiment_states instead of Tkinter variables
			for experiment_name, enabled in self.experiment_states.items():
				if enabled and experiment_name in self.experiments:
					# Create a deep copy with primitive data only
			...
			# CRITICAL FIX: Set total_experiments BEFORE starting thread
			...
			# Start thread with primitive data only (NO Tkinter objects)
	...
	'''	def _run_experiments_thread(self, experiments_list, s2t_config, options):
			"""FIXED: Run experiments in isolated thread with NO UI dependencies"""
			try:
				total_experiments = len(experiments_list)

				# CRITICAL: Set up experiment tracking FIRST
				...
				# Update framework options based on UI settings
				if self.Framework:
				...
				for index, exp_data in enumerate(experiments_list):
					if self.cancel_requested.is_set():
						self.log_status(f"Execution cancelled before experiment {index + 1}")
						break
					
					experiment_name = exp_data['experiment_name']

					# CRITICAL: Update experiment index BEFORE starting experiment
					self.main_thread_handler.queue_status_update({
					...
					# Small delay to ensure index update is processed before Framework starts
					time.sleep(0.1)
					
					# FIXED: Update experiment status through main thread handler
					self.main_thread_handler.queue_status_update({
					...
					try:
						...
						self.log_status(f"Experiment '{experiment_name}' failed with exception: {str(e)}")
						success = False

					# Check for cancellation after each experiment
					if self.cancel_requested.is_set():
				...
				try:
					...
					# Clear any remaining references that might hold Tkinter variables
					experiments_list.clear()
					s2t_config.clear()
					options.clear()
					
					# Queue final completion through main thread handler
					self.main_thread_handler.queue_status_update({
	...
	'''def _force_cleanup_after_cancel(self):
			"""Force cleanup even if thread hasn't finished"""
			try:
				...
				if self.framework_thread and self.framework_thread.is_alive():
					self.log_status("Warning: Background thread still running after cancel")
				
			except Exception as e:
				self.log_status(f"Error in force cleanup: {e}")'''
	...
	'''	def _complete_cancel_cleanup(self):
			...
			try:
				...
				self.framework_execution_active = False
				self.current_experiment_index = 0
				self.thread_active = False  # IMPORTANT: Reset thread flag
				
				if hasattr(self, 'end_after_current'):
					self.end_after_current = False
				
				# Clear the cancel flag SAFELY
				#self._safe_clear_cancel_flag()
	...
	'''
	'''	def _safe_clear_cancel_flag(self):
			"""Safely clear cancel flag"""
			try:
				if hasattr(self, 'cancel_requested'):
					self.cancel_requested.clear()
				
				# Clear framework cancel flag if it exists
				if self.Framework and hasattr(self.Framework, 'cancel_flag'):
					if self.Framework.cancel_flag:
						self.Framework.cancel_flag.clear()
						
			except Exception as e:
				print(f"Error clearing cancel flag: {e}")'''
	...
	def on_closing(self):
		...
		try:
			...
			if self.framework_thread and self.framework_thread.is_alive():
				self.framework_thread.join(timeout=3.0)
