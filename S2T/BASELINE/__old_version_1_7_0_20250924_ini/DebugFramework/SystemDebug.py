	...
	def execute_single_test(self) -> TestResult:
		...
		try:
			...
			serial = ser.teraterm(
				...
				framework=self.framework # Passing Framework object
			)
				...
				if halt_controller:
					...
					if halt_controller.should_end_after_iteration():
						...
						halt_controller._send_status_update('experiment_ended_by_command', {
							...
							'total_iterations': self.loops,  # or total_tests for other strategies
			...
			for i, value in enumerate(self.values):
				...
				if halt_controller:
					...
					if halt_controller.should_end_after_iteration():
						...
						halt_controller._send_status_update('experiment_ended_by_command', {
							'completed_iterations': i + 1,
							'total_iterations': total_tests,  # or total_tests for other strategies
							'reason': 'END command received after iteration completion',
							'final_result': result.status,
							'experiment_name': getattr(executor.config, 'experiment_name', executor.config.name)
						})
...
class Framework:
	...
	def end_experiment(self):
		...
		if success:
			...
			if self._status_reporter:
				self._send_status_update('experiment_end_requested', {
					'test_name': self.config.name,
					'experiment_name': getattr(self.config, 'experiment_name', self.config.name),
	...
	def _prepare_for_new_execution(self):
		...
		try:
			...
			if self._status_reporter:
				self._send_status_update('execution_prepared', {
	...
	def _finalize_execution(self, reason="completed"):
		...
		try:
			...
			# Send final status
			if self._status_reporter:
				self._send_status_update('execution_finalized', {
					'reason': reason,
					'timestamp': time.time()
				})
	...
	def _send_status_update(self, status_type: str, data: Dict[str, Any]):
		"""Queue status update instead of direct callback"""
		...
		try:
			...
			if status_type in ['strategy_progress', 'iteration_progress']:
				progress_info = data.get('progress_percent') or data.get('progress_weight', 0) * 100
				print(f"DEBUG Framework Status: {status_type} - {progress_info:.1f}%")
						
			# Use queue instead of direct callback
			self._status_reporter.report_status(status_data)

		except Exception as e:
			# Don't let status updates break execution
			print(f"Status update queued with error: {e}")

	# Method to get status updates
	def get_status_updates(self) -> list:
