...
class teraterm():
	...
	def __init__(self, visual, qdf, bucket, log, cmds, tfolder, test, ttime, tnum, DebugLog, chkcore=None, content='Dragon', host = None, PassString = 'Test Complete', FailString = 'Test Failed', cancel_flag = None, framework = None):
	...
	def check_user_cancel(self):
		...
		if self.framework != None:
			...
			if command_result == "CANCELLED":  # Handle CANCELLED status
				self.DebugLog("CANCEL command received - stopping execution", 2)
				self.framework.config.execution_cancelled = True
				raise InterruptedError("SERIAL: Execution cancelled")		

		# Old Method -- Used by TTL Test	
		elif cancel_check:
