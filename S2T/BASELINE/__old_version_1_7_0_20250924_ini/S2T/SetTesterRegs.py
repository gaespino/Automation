...
def MeshQuickTest(core_freq = None, mesh_freq = None, vbump_core = None, vbump_mesh = None, Reset = False, Mask = None, pseudo = False, dis_2CPM = None, GUI = True, fastboot = True, corelic = None, volttype='vbump', debug= False, boot_postcode = False, extMask = None, u600w=None, cancel_flag=False):
	s2tTest = S2TFlow(debug=debug)
	product = SELECTED_PRODUCT
	qdf = dpm.qdf_str()
	
	# Framework cancellation check enable
	s2tTest.cancel_flag = cancel_flag
...
def SliceQuickTest(Target_core = None, core_freq = None, mesh_freq = None, vbump_core = None, vbump_mesh = None, Reset = False, pseudo = False, dis_2CPM = None, GUI = True, fastboot = True, corelic = None,  volttype = 'fixed', debug= False, boot_postcode = False, u600w=None, cancel_flag=False):
	s2tTest = S2TFlow(debug=debug)
	product = SELECTED_PRODUCT
	qdf = dpm.qdf_str()
...
class S2TFlow():
	...
	def powercycle(self):
		...
		time.sleep(scm.EFI_POSTCODE_WT)
		scm._wait_for_post(scm.EFI_POST, sleeptime=scm.EFI_POSTCODE_WT, additional_postcode=scm.LINUX_POST, cancel_flag = self.cancel_flag)
		scm.svStatus(refresh=True, cancel_flag = self.cancel_flag)
	...
	def slice_run(self):
		...
		slice_mode = scm.System2Tester(target = self.targetLogicalCore, masks = self.masks, boot=True, ht_dis=False, dis_2CPM = self.dis_2CPM, fresh_state= False,fastboot = self.fastboot, ppvc_fuses=self.volt_config, cancel_flag = self.cancel_flag)

		slice_mode.setCore()
		
		# Will force refresh after boot to properly load all the nodes after mask changes
		scm.svStatus(refresh=True, cancel_flag = self.cancel_flag) #ipc.unlock(); ipc.forcereconfig(); sv.refresh()
	...
	def slice_init(self):
		...
		scm.svStatus(refresh=False, cancel_flag = self.cancel_flag)
		self.init_flow()
	...
	def slice_end(self):
		...
		scm.svStatus(cancel_flag = self.cancel_flag)
	...
	def mesh_run(self):
		...
		mesh = scm.System2Tester(target= self.target, masks = self.masks, boot = True, ht_dis = self.dis_ht, dis_2CPM = self.dis_2CPM, fresh_state = False, readFuse = True, clusterCheck = self.clusterCheck,fastboot = self.fastboot, ppvc_fuses=self.volt_config, cancel_flag = self.cancel_flag)
