...
#========================================================================================================#
#=============== Framework Utilities Scripts ============================================================#
#========================================================================================================#
try:
	from users.gaespino.dev.DebugFramework.ExecutionHandler.utils.FrameworkUtils import FrameworkUtils
	
	FRAMEWORK_UTILS_READY = True
except:
	print(' -- WARNING: Unable to import Framework Utils Module --')
	FRAMEWORK_UTILS_READY = False
...
class System2Tester():
	...
	def __init__(self, target, masks = None, coremask=None, slicemask=None, boot = True, ht_dis = False, dis_2CPM = None, fresh_state = True, readFuse = False, clusterCheck = True , fastboot = True, ppvc_fuses = None, cancel_flag = False):
		...
		svStatus(cancel_flag=self.cancel_flag)
	...
	'''
		...
		if global_dry_run == False:
			...
			fuse_cmd_override_reset(fuse_cmd_array=_fuse_str, s2t=True, cancel_flag=self.cancel_flag)

			if (stop_after_mrc):

				_wait_for_post(AFTER_MRC_POST, sleeptime=MRC_POSTCODE_WT, timeout = MRC_POSTCODE_CHECK_COUNT, cancel_flag=self.cancel_flag)
			if (boot_postcode):

				_wait_for_post(BOOT_STOP_POSTCODE, sleeptime=BOOT_POSTCODE_WT, timeout = BOOT_POSTCODE_CHECK_COUNT, additional_postcode=LINUX_POST, cancel_flag=self.cancel_flag)
						
			else:
				_wait_for_post(EFI_POST, sleeptime=EFI_POSTCODE_WT, timeout = EFI_POSTCODE_CHECK_COUNT, additional_postcode=LINUX_POST, cancel_flag=self.cancel_flag)
			sv_refreshed = False
		...
		while attempt < n:
			...
			try:
				...
				if 'RSP 11 - Multicast Mixed stats' in str(e):
					...
					svStatus(checkipc=True, checksvcores=False, refresh=False, reconnect=False,cancel_flag=self.cancel_flag)
...
def fuse_cmd_override_reset(fuse_cmd_array, skip_init=False, boot = True, s2t=False, cancel_flag=None):
	sv = _get_global_sv()
	ipc = ipccli.baseaccess()
	...
	if skip_init==False:
		...
		#sv.refresh()
		#cd.refresh()
		svStatus(refresh = (False if s2t else True),cancel_flag=cancel_flag)
	...
	if boot: 
		...
		if not s2t:
			...
			_wait_for_post(EFI_POST, sleeptime=EFI_POSTCODE_WT, timeout=EFI_POSTCODE_CHECK_COUNT, additional_postcode= LINUX_POST, cancel_flag=cancel_flag)
...
def svStatus(checkipc = True, checksvcores = True, refresh = False, reconnect = False, cancel_flag=None):
	from namednodes import sv
	ipc = ipccli.baseaccess()
	SysStatus = []
	ipcBad = False
...
def check_user_cancel(cancel_flag=False):
	if FRAMEWORK_UTILS_READY and cancel_flag:
		FrameworkUtils.check_cancellation("System 2 Tester Flow")

def clear_cancel_flag(logger=None,cancel_flag=False):
	if FRAMEWORK_UTILS_READY and cancel_flag:
		#FrameworkUtils.check_cancellation("System 2 Tester Flow")
		FrameworkUtils.clear_cancellation(logger=logger, context="System 2 Tester Flow")
'''
...
def go_to_efi(cancel_flag = False):
	print(Fore.YELLOW +'\n>>> Reenabling boot going to EFI ---\n')
	svStatus(cancel_flag=cancel_flag)
...
def _wait_for_post(postcode, sleeptime = 3, timeout = 10, additional_postcode = None, cancel_flag=None):
	done=False
	...
	while(not done):
		...
		check_user_cancel(cancel_flag=cancel_flag)
