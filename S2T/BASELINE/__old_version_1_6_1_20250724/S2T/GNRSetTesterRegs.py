...
import users.THR.PythonScripts.thr.S2T.GNRCoreManipulation as scm
import users.THR.PythonScripts.thr.S2T.GNRGetTesterCurves as stc
...
## UI Calls
import users.THR.PythonScripts.thr.S2T.UI.GNRSystem2TesterUI as UI
...
class S2TFlow():
	...
	def init_flow(self):
		...
		if self.reset_start:
			...
			#time.sleep(60)
			#scm._wait_for_post(scm.EFI_POST, sleeptime=60)
	...
	def powercycle(self):
		...
		# Wait time to start reading PythonSV again
		time.sleep(60)
		scm._wait_for_post(scm.EFI_POST, sleeptime=60, additional_postcode=scm.LINUX_POST)
		scm.svStatus(refresh=True)
