...
# Imports from THR S2T scripts
import users.THR.PythonScripts.thr.GNRFuseOverride as f
import users.THR.PythonScripts.thr.S2T.GNRCoreManipulation as gcm
import users.THR.PythonScripts.thr.S2T.Logger.ErrorReport as dpmtileview
import users.THR.PythonScripts.thr.S2T.Logger.logger as dpmlog
import users.THR.PythonScripts.thr.S2T.Tools.portid2ip as p2ip
import users.THR.PythonScripts.thr.S2T.ConfigsLoader as LoadConfig
...
def logger(visual = '', qdf = '', TestName='', Testnumber = 0, dr_dump = True, chkmem = 0, debug_mca = 0, folder=None, WW = '', Bucket = 'UNCORE', UI=False, refresh = False, logging = None):
	...
	if UI: dpmlog.callUI(qdf = qdf, ww = WW, product = product)   		
		...
		try:
			dpmtileview.run(visual, Testnumber, TestName, chkmem, debug_mca, dr_dump = dr_dump, folder = folder, WW = WW, Bucket = Bucket, product = product, QDF = qdf, logger = logging)
...
def ppvc(bsformat = False, ppvc_fuses = [], updateram=False):
	...
	try: rgb_values = f.get_rgb_values(qdf)
	except: 
