...
try:
    from users.gaespino.DebugFramework.Storage_Handler.DBClient import DB_Client
    import users.gaespino.DebugFramework.Storage_Handler.ReportUtils as reportUtils
    import users.gaespino.DebugFramework.Storage_Handler.MongoDBCredentials as pqe_mongo_db_credentials
except:
...
def upload_summary_report_bck(storage_dir,logger_name="FrameworkLogger"):
    try:
        file_path=reportUtils.find_excel_files(storage_dir)[0]
    except:
        raise Exception(f"Error: Summary file not found in directory '{storage_dir}'")
    ...
    splitted_path = os.path.normpath(file_path).lstrip("\\/").split(os.sep)
