...
class EnhancedThreadSafeState:
	...
	def issue_command(self, command: ExecutionCommand, data: Optional[Dict[str, Any]] = None, 
		...
		# ✅ Special debug tracing for critical commands
		if command in [ExecutionCommand.CANCEL, ExecutionCommand.END_EXPERIMENT]:
			import traceback
			thread_name = threading.current_thread().name
			timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]
			
			self._debug_log(f"🚨 CRITICAL COMMAND ISSUED: {command.name}")
			self._debug_log(f"   Thread: {thread_name}")
			self._debug_log(f"   Timestamp: {timestamp}")
			self._debug_log(f"   Data: {data}")
			self._debug_log(f"   Callback: {callback is not None}")
			
			# Show detailed stack trace for these critical commands
			stack = traceback.format_stack()
			self._debug_log(f"   Stack trace for {command.name} command:")
	...
	def should_stop(self) -> bool:
		...
		with self._lock:
			...
			self._debug_log(f"Should stop: TRUE (Cancel: {cancel}, End: {end} & step_mode {is_step_mode}, Emergency: {emergency})")
