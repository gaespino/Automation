...
class TestUpload:
	...
	def upload_data(self):
		...
		UPLOAD_DATA = self.upload_to_danta
		tfolder = self.folder if self.from_Framework else self.TemporalFolder
		## Debug Framework Disk
		DATA_SERVER = r'\\crcv03a-cifs.cr.intel.com\mfg_tlo_001'
		DATA_DESTINATION = rf'{DATA_SERVER}\DebugFramework\{self.product}'
			
		if self.upload_to_disk: 
			self.print('Copying Data to Framework disk --- \n')
			dest_folder=copy_folder(src=tfolder, dest=DATA_DESTINATION, visual=self.vid, zipdata=True , logger = self.print)
		
		if DATABASE_HANDLER_READY and UPLOAD_DATA:
			try:
				self.print('Uploading Data to Danta Server --- \n')
				db.upload_summary_report(storage_dir=dest_folder)
			except Exception as e:
				self.print(f' Failed updloading data to DantaDB -- Exception {e}',2)
	...
	def Iteration_end(self, tnum, runName, runStatus, scratchpad, seed):
		...
		#self.print(print_custom_separator(f'Test iteration summary'))
		#self.print(f' -- Test Name: {self.runName} --- ')
		#self.print(f' -- Test Result: {self.runStatus} --- ')
		...
		#self.print(f' -- System2TesterConfig: {s2t_config_path} --- ')
