...
class DebugFrameworkControlPanel:
	...
	def __init__(self, root, Framework):
		...
		# Initialize Framework with thread-safe callback
		if Framework:
			self.Framework = Framework(status_callback=self._thread_safe_status_callback)
		else:
			self.Framework = None

	def _disable_status_callbacks_temporarily(self, duration_ms=2000):
		"""Temporarily disable status callbacks to prevent threading issues"""
		self._status_callback_enabled = False
		self.root.after(duration_ms, lambda: setattr(self, '_status_callback_enabled', True))

	def _thread_safe_status_callback(self, status_data):
		"""Thread-safe wrapper for status callbacks"""
		try:
			# Queue the update through MainThreadHandler
			self.main_thread_handler.queue_status_update(status_data)
		except Exception as e:
			print(f"Status callback error (suppressed): {e}")
	...
	def on_closing(self):
		...
		try:
			# Disable callbacks immediately
			if hasattr(self, 'main_thread_handler'):
				self.main_thread_handler.disable_callbacks()
				self.main_thread_handler.clear_queue()
			
			# Cancel all operations
			self.cancel_requested.set()
			
			# Stop framework with timeout
			if self.Framework and self.framework_execution_active:
				try:
					# Disable callback first
					self.Framework.status_callback = None
					...
					print(f"Error stopping framework during close: {e}")
			...
			if self.Framework and hasattr(self.Framework, '_pending_status_updates'):
				self.Framework._pending_status_updates.clear()
	...
	def _update_ui_from_status(self, status_type, data):
		"""Update UI elements based on framework status (runs in main thread)"""
		try:
			if status_type == 'experiment_start':
				# New experiment started
				self.current_experiment_name = data['experiment_name']
				self.total_iterations_in_experiment = data['total_iterations']
				self.current_iteration = 0
				self.current_iteration_progress = 0.0
				
				self.log_status(f"Started Experiment: {data['experiment_name']}")
				self.log_status(f"Strategy: {data['strategy_type']} with {data['total_iterations']} iterations")
			...
			elif status_type == 'iteration_start':
				self.current_iteration = data['iteration']
				self.current_iteration_progress = data.get('progress_weight', 0.0)
				self.log_status(f"Iteration {data['iteration']}: {data['status']}")
				self._update_overall_progress()
				
			elif status_type == 'iteration_progress':
				self.current_iteration = data['iteration']
				self.current_iteration_progress = data.get('progress_weight', 0.0)
				self.log_status(f"Iteration {data['iteration']}: {data['status']}")
				self._update_overall_progress()
				
			elif status_type == 'iteration_complete':
				self.current_iteration = data['iteration']
				self.current_iteration_progress = 1.0
				
				status_msg = f"Iteration {data['iteration']}: {data['status']}"
				if data.get('scratchpad'):
					status_msg += f" ({data['scratchpad']})"
				self.log_status(status_msg)
				
				self._update_overall_progress()
				self.update_progress_display(result_status=data['status'])
				
			elif status_type == 'iteration_failed' or status_type == 'iteration_cancelled':
				self.current_iteration = data['iteration']
				self.current_iteration_progress = 1.0
				
				error_msg = f"Iteration {data['iteration']}: {data['status']}"
				if data.get('error'):
			...
			elif status_type == 'strategy_progress':
				# Update progress from strategy
				...
				iteration_text = f"Iter {self.current_iteration}/{total_iterations}"
			...
			elif status_type == 'strategy_execution_complete':
				self._display_experiment_summary(data)
				self.log_status(f"Strategy execution completed")
			...
			elif status_type == 'experiment_complete':
				self._display_experiment_summary(data)
				self.log_status(f"Experiment '{data['test_name']}' completed successfully")
				
			elif status_type == 'experiment_failed':
				self.log_status(f"Experiment '{data['experiment_name']}' failed: {data['reason']}")
				
			# Handle END/CANCEL status updates with minimal UI changes
			elif status_type == 'experiment_end_requested':
				self.log_status(f"END command received - experiment will stop after current iteration")
				# Don't update UI here - let the test loop handle it
				
			elif status_type == 'experiment_ended_by_command':
				completed = data['completed_iterations']
				total = data['total_iterations']
				reason = data['reason']
				self.log_status(f"Experiment ended by {reason} after {completed}/{total} iterations")
				# Don't update UI here - let the test loop handle it
				
			elif status_type == 'execution_cancelled':
				self.log_status(f"Execution cancelled")
				# Don't update UI here - let the test loop handle it
				
		except Exception as e:
			print(f"Error updating UI from status: {e}")
	...
	def _update_overall_progress(self):
		...
		if self.total_iterations_in_experiment > 0:
			...
			# Total progress within current experiment
			experiment_progress = completed_iterations_progress + current_iteration_progress
		else:
	...
	def toggle_framework_hold(self):
		"""Toggle framework halt/continue functionality"""
		if self.Framework and self.framework_execution_active:
			if not self.Framework._is_halted:
				# Halt the framework
				self.Framework.halt_execution()
				self.hold_button.configure(text="Continue", style="Continue.TButton")
				self.log_status("Framework execution halted")
				self.status_label.configure(text=" Halted ", bg="orange", fg="black")
			else:
				# Continue the framework
				self.Framework.continue_execution()
				self.hold_button.configure(text="Hold", style="Hold.TButton")
				self.log_status("Framework execution resumed")
				self.status_label.configure(text=" Running ", bg="#BF0000", fg="white")
		else:
			# Fallback to old hold functionality for non-framework tests
			self.hold_active = not self.hold_active
			if self.hold_active:
				self.hold_button.configure(text="Resume", style="HoldActive.TButton")
				self.log_status("Test sequence halted")
			else:
				self.hold_button.configure(text="Hold", style="Hold.TButton")
				self.log_status("Test sequence resumed")
	...
	def create_experiment_rows(self):
		"""Create experiment rows with proper sizing and background"""
		...
		for experiment_name, experiment_data in self.experiments.items():
			...
			checkbox = ttk.Checkbutton(frame, variable=enabled_var, 
									command=lambda f=frame, v=enabled_var: self.toggle_experiment(f, v))
			...
			# Status label - Use tk.Label for color support with proper background
	...
	def start_tests_thread(self):
		"""Enhanced start with proper cleanup"""
		# CRITICAL: Ensure previous execution is completely stopped
		self._ensure_previous_execution_stopped()
		...
		# IMPORTANT: Reset Framework state BEFORE starting
		if self.Framework:
			# Reset Framework state properly
			...
			if hasattr(self.Framework, 'cancel_flag'):
				self.Framework.cancel_flag = None
			# Ensure callback is properly set
			self.Framework.status_callback = self._thread_safe_status_callback
		
		self._reset_end_command_state()
		self.reset_timing()
		self.reset_progress_tracking()
		
		# Update UI state
		self._update_ui_for_execution_start()
				...
				try:
					if self.Framework == None:
						...
						success = execute(task_name = test_mode, target_func=Framework_call, 
											exceptions = self.exception_queue, cancel_flag = self.cancel_requested, 
	...
	def cancel_tests(self):
		...
		try:
			self.log_status("Cancellation requested - stopping all tests")
			
			# Set cancellation flag first
			self.cancel_requested.set()
			
			# Cancel framework execution if active
			if self.Framework and self.framework_execution_active:
				try:
					# Temporarily disable status updates during cancel
					self.main_thread_handler.disable_callbacks_temporarily(5000)  # 5 seconds
					self.Framework.cancel_execution()
					self.log_status("Framework execution cancelled")
				except Exception as e:
					self.log_status(f"Error cancelling framework: {e}")
			...
			self.root.after(2000, self._complete_cancel_cleanup)
			
		except Exception as e:
			self.log_status(f"Error during cancellation: {e}")

	def _complete_cancel_cleanup(self):
		"""Complete cancellation cleanup after delay"""
		try:
			# Reset states
			self.framework_execution_active = False
			self.current_experiment_index = 0
			...
			self.root.after(1000, self._reset_buttons_after_cancel)
	...
	def end_current_experiment(self):
		...
		try:
			self.log_status("END command pressed")
			
			if self.Framework and self.framework_execution_active:
				try:
					success = self.Framework.end_experiment()
					if success:
						self.log_status("END command sent to framework")
						# Simple UI update - just disable button
						self.end_button.configure(text="Ending...", state=tk.DISABLED)
					else:
						self.log_status("Failed to send END command - no experiment running")
				except Exception as e:
					self.log_status(f"Error sending END command: {e}")
					
			else:
				# Fallback for non-framework tests
				if hasattr(self, 'end_after_current'):
					self.end_after_current = True
					self.end_button.configure(text="Ending...", state=tk.DISABLED)
					self.log_status("END command set for non-framework tests")
				else:
					self.log_status("END command not available for current test type")
					
		except Exception as e:
			self.log_status(f"Error processing END command: {e}")
