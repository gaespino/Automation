...
def teraterm_check(com_port, ip_address='192.168.0.2', teraterm_path = r"C:\teraterm", seteo_h_path = r"C:\SETEO_H", ini_file = "TERATERM.INI", useparser=False, checkenv=False):
	...
	if not os.path.exists(ini_file_path):
		...
		if os.path.exists(source_folder):
			...
			print("teraterm folder not found in C:\\SETEO_H.")
			exit()
	...
	init_host_variables = { 'FrameworkSerial': f'{com_port}',
	...
	if checkenv:
		...
		check_and_update_env_variables(init_host_variables)

def check_and_update_env_variables(variables):
    for var_name, desired_value in variables.items():
        ...
        if current_value is None:
            ...
            os.environ[var_name] = desired_value
            print(f"Updated environment variable '{var_name}' from '{current_value}' to '{desired_value}'.")
        else:
            # Environment variable exists and has the correct value
            print(f"Environment variable '{var_name}' already exists with the correct value '{current_value}'.")
