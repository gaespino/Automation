...
class teraterm():
	...
	def __init__(self, visual, qdf, bucket, log, cmds, tfolder, test, ttime, tnum, DebugLog, chkcore=None, content='Dragon', host = None, PassString = 'Test Complete', FailString = 'Test Failed', cancel_flag = None):
	...
	def checker(self):
		...
		if tstPass: 
			self.DebugLog('Test Completed Succesfully')
			tname = f'{self.test}_PASS'
			self.testresult = f'PASS::{self.test}'
			
		else: 
			self.DebugLog('Test Failed')
...
def start(visual='-999999999', qdf='Test', bucket='UNCORE', content = 'Dragon', host = '192.168.0.2', log=log_file_path, cmds=macrospath, tfolder=tfolder, test='Dummy', ttime=30, tnum=1, PassString = 'Test Complete', FailString = 'Test Failed'):
	macro_files = {
	...
	tst = teraterm(visual, qdf, bucket, log, macro_files, tfolder, test, ttime, tnum, DebugLog, chkcore=None, content=content, host = host, PassString = PassString, FailString = FailString)
	tst.run()
