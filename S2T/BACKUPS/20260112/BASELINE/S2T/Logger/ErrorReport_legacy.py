...
REFACTORED: Now uses ErrorReportGenerator class for multi-product support
This file maintains backward compatibility with existing code while delegating
to the new class-based implementation in ErrorReportClass.py
"""
...
try:
	if SELECTED_PRODUCT == 'GNR':
		...
		mca_banks = None
		mca_dump = None
		print(f'MCA bank decoder not available for {SELECTED_PRODUCT}')
except Exception as e:
	mca_banks = None
	mca_dump = None
	print(f'Error importing MCA bank decoder: {e}')
...
try: 
	if SELECTED_PRODUCT == 'GNR':
		...
		import mc.dmr_DimmInfo as Dimm ## Not available for DMR at the moment
except: 
...
if SELECTED_PRODUCT == 'GNR' or SELECTED_PRODUCT == 'CWF':
...
if SELECTED_PRODUCT == 'DMR':
	from toolext.server_ip_debug.ccf import ccf
	from pysvtools.server_ip_debug.pm import pm
	from toolext.server_ip_debug.punit import punit
	from pysvtools.server_ip_debug.ubox import ubox
	from toolext.server_ip_debug.ula import ula
	import diamondrapids.tools.tileview.tileview_dmr as t
...
def generate_frameworklog(handler, frame_vars):
	...
	dis1CPM = frame_vars.get('dis1CPM', False)
	...
	handler.initlog()
	handler.TestBanner(qdf, tnum, mask, corelic, bumps, htdis, dis1CPM, dis2CPM, freqIA, voltIA, freqCFC, voltCFC, content, passstring, failstring)
...
def run(visual ='', Testnumber='', TestName='', chkmem = 0, debug_mca = 0, dr_dump = False, folder=None, WW = 'WW', Bucket = 'UNCORE', product = 'GNR', QDF = '', logger =None, upload_to_disk = False, upload_to_danta = False, framework_data = None):
	...
	REFACTORED: Now delegates to ErrorReportGenerator class for multi-product support.
	All legacy functions below are kept for reference but the main execution uses the new class.
	"""
	...
	try:
		...
		print(f"Error using new ErrorReportGenerator class: {e}")
		print("Falling back to legacy implementation...")
		# Fall back to legacy implementation if new class fails
		return run_legacy(visual, Testnumber, TestName, chkmem, debug_mca, dr_dump, 
						 folder, WW, Bucket, product, QDF, logger, upload_to_disk, 
						 upload_to_danta, framework_data)


def run_legacy(visual ='', Testnumber='', TestName='', chkmem = 0, debug_mca = 0, dr_dump = False, folder=None, WW = 'WW', Bucket = 'UNCORE', product = 'GNR', QDF = '', logger =None, upload_to_disk = False, upload_to_danta = False, framework_data = None):
	"""
	LEGACY implementation - kept for backward compatibility and reference
	Main function of the script
	"""
...
def decode_mca_bank(register_path, status_value=None):
	...
	Returns:
		str: Formatted bank information or empty string if decoder unavailable
	"""
	if mca_banks is None:
		return ""
	
	return mca_banks.decode_mca_bank(register_path, status_value)


def mca_init():
	"Perform MCA unlock and socket refresh for GNR"
...
def mca_dump_gnr(verbose=True):
	...
	"""
	# Init the SV and IPC connection
	...
	return mca_dump(sv, itp=itp, verbose=verbose)


# Legacy implementation kept for reference (can be removed after verification)
def mca_dump_gnr_old(verbose=True):
...
# Legacy implementation kept for reference (can be removed after verification)
def mca_dump_cwf_old(verbose=True):
...
# Legacy implementation kept for reference (can be removed after verification)
def mca_dump_dmr_old(verbose=True):
