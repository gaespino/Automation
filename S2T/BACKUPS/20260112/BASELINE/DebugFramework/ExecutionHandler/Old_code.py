...
class defeature():
	...
	def Test(self):
		...
		# Not waiting for postcode in Units of 600W -- RegAcc will end up failing
		# Note: The Reset will be set to True in case of 600W units we need to get to a registers read point
		wait_postcode = not(self.u600w)
		self.Reset = self.Reset if self.u600w == False else True
		...
		if self.ismesh:
			...
			try: 
				...
				if 'RSP 10' in str(e) or 'regaccfail' in str(e):
					...
					self.powercycle(u600w = False, wait_postcode=wait_postcode) # Removing the 600W portion
					time.sleep(120)
			...
			try:
				...
				if 'RSP 10' in str(e) or 'regaccfail' in str(e):
					#self.powercycle(u600w = self.u600w, wait_postcode=False) # Removing the 600W portion
					self.powercycle(u600w = False, wait_postcode=wait_postcode)
					time.sleep(120)
					Framework.reconnect_ipc()		
				else:
					self.powercycle(u600w = False, wait_postcode=wait_postcode) # Removing the 600W portion
