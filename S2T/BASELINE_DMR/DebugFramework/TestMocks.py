...
def setup_all_mocks(product: str = "GNR"):
    ...
    # Inject mocks into FrameworkUtils if it's already been imported
    try:
        import DebugFramework.ExecutionHandler.utils.FrameworkUtils as fut

        # Inject the mock modules into FrameworkUtils global namespace
        fut.dpm = mock_dpm_checks
        fut.gcm = mock_core_manipulation
        fut.s2t = mock_set_tester_regs
        fut.s2tutils = mock_s2t_utils
        fut.ser = sys.modules.get('users.gaespino.dev.DebugFramework.SerialConnection')
        fut.fh = sys.modules.get('users.gaespino.dev.DebugFramework.FileHandler')
        fut.gme = sys.modules.get('DebugFramework.MaskEditor')

        print(f"[MOCK SETUP] Injected mocks into FrameworkUtils module")
    except ImportError:
        print(f"[MOCK SETUP] FrameworkUtils not yet imported, mocks will be available when it imports")
    except Exception as e:
        print(f"[MOCK SETUP] Warning: Could not inject mocks into FrameworkUtils: {e}")
...
"""
        ...
        def load_json(file_path):
            """Mock load JSON - alias for load_json_file"""
            return MockFileHandler.load_json_file(file_path)
        ...
        @staticmethod
        def ini_to_dict_with_types(ini_file_path):
            """Mock INI to dict conversion with type inference"""
            print(f"[MOCK FH] Mock converting INI to dict with types: {ini_file_path}")

            # Return mock configuration dict
            return {
                'EXECUTION_CONTROL': {
                    'flow_type': 'SLICE',
                    'command_timeout': 30
                ...
                'DRAGON_CONFIG': {
                    ...
                    'frequency': 2400,
                    'voltage': 1.0,
                    'content_path': 'FS1:\\content\\Dragon\\',
                    'enabled': True
                },
                'MESH_CONFIG': {
                    'mesh_frequency': 1800,
...
class MockDPMChecks:
        ...
        @staticmethod
        def get_cbb_index(core):
            """Mock get CBB index for DMR"""
            print(f"[MOCK DPM] Getting CBB index for core {core}")
            return 0  # Default CBB index

        @staticmethod
        def get_single_compute_apic_location(core):
            """Mock get APIC location for DMR"""
            print(f"[MOCK DPM] Getting APIC location for core {core}")
            return "0x0"  # Default APIC location

        @staticmethod
        def hwls_miscompare(logger=None):
            """Mock HWLS miscompare check for CWF"""
            if logger:
                logger("[MOCK DPM] HWLS miscompare check (CWF)", 1)
            else:
                print("[MOCK DPM] HWLS miscompare check (CWF)")
            return True
...
class MockSerialConnection:
    """Mock SerialConnection module with teraterm class"""
    log_file_path = r"C:\Temp\capture.log"  # Match real Framework expectation

    # The teraterm class that will be instantiated
    teraterm = MockTeraterm

    # Module-level constants from SerialConnection.py
    TERATERM_PATH = r'C:\teraterm'
    TERATERM_MACRO_PATH = r'C:\teraterm\ttermpro.exe'
...
class MockS2TUtils:
    ...
    def formatException(e):
        ...
        tb = ''.join(tb_lines)
        print(f"\n[MOCK UTILS DEBUG] Exception formatting:")
        print(f"Error: {error_msg}")
        print(f"Full Traceback:\n{tb}")
        print(f"[MOCK UTILS DEBUG] End of traceback\n")
...
# Mock MaskEditor
class MockMaskEditor:
    """Mock for MaskEditor module"""

    @staticmethod
    def DebugMask(basemask=None, root=None, callback=None):
        """Mock DebugMask creation"""
        print(f"[MOCK MaskEditor] Creating DebugMask with basemask={basemask}")
        return {"mock_mask": "created"}
...
mock_dpm_checks = MockDPMChecks()
mock_core_manipulation = MockCoreManipulation()
mock_configs_loader = MockConfigsLoader()
mock_s2t_utils = MockS2TUtils()
mock_set_tester_regs = MockSetTesterRegs()
mock_serial_connection = MockSerialConnection()
mock_file_handler = MockFileHandler()
mock_mask_editor = MockMaskEditor()

sys.modules['dpmChecks'] = mock_dpm_checks
sys.modules['CoreManipulation'] = mock_core_manipulation
sys.modules['ConfigsLoader'] = mock_configs_loader

# DO NOT register DebugFramework as a module - let Python find the real directory
# Only pre-register the specific submodules that need mocking
# When SystemDebug imports "import DebugFramework.SerialConnection",
# Python will use our mock from sys.modules instead of loading the real file
sys.modules['DebugFramework.SerialConnection'] = mock_serial_connection
sys.modules['DebugFramework.FileHandler'] = mock_file_handler
sys.modules['DebugFramework.MaskEditor'] = mock_mask_editor

# Create S2T module hierarchy (for DEV_MODE imports like 'import S2T.dpmChecks as dpm')
S2T = types.ModuleType('S2T')
S2T.CoreManipulation = mock_core_manipulation
S2T.dpmChecks = mock_dpm_checks
S2T.SetTesterRegs = mock_set_tester_regs
S2T.Tools = types.ModuleType('S2T.Tools')
S2T.Tools.utils = mock_s2t_utils
...
users.gaespino.dev.S2T.CoreManipulation = mock_core_manipulation
users.gaespino.dev.S2T.dpmChecks = mock_dpm_checks
users.gaespino.dev.S2T.SetTesterRegs = mock_set_tester_regs  # This is the critical one
users.gaespino.dev.S2T.Tools.utils = mock_s2t_utils
users.gaespino.dev.DebugFramework.SerialConnection = mock_serial_connection
users.gaespino.dev.DebugFramework.FileHandler = mock_file_handler
...
users.THR.PythonScripts.thr.S2T.CoreManipulation = mock_core_manipulation
users.THR.PythonScripts.thr.S2T.dpmChecks = mock_dpm_checks
users.THR.PythonScripts.thr.S2T.SetTesterRegs = mock_set_tester_regs
users.THR.PythonScripts.thr.S2T.Tools.utils = mock_s2t_utils
...
print(f"users.THR.PythonScripts.thr.S2T.dpmChecks available: {'users.THR.PythonScripts.thr.S2T.dpmChecks' in sys.modules}")
print(f"GNRCoreDebugUtils mock available: {users.THR.PythonScripts.thr.GNRCoreDebugUtils}")

# DEBUG: Print available mock methods
print(f"\n[MOCK DEBUG] mock_dpm_checks type: {type(mock_dpm_checks)}")
print(f"[MOCK DEBUG] mock_dpm_checks has get_cbb_index: {hasattr(mock_dpm_checks, 'get_cbb_index')}")
print(f"[MOCK DEBUG] mock_dpm_checks has get_single_compute_apic_location: {hasattr(mock_dpm_checks, 'get_single_compute_apic_location')}")
print(f"[MOCK DEBUG] mock_dpm_checks has hwls_miscompare: {hasattr(mock_dpm_checks, 'hwls_miscompare')}")
print(f"[MOCK DEBUG] mock_dpm_checks methods: {[m for m in dir(mock_dpm_checks) if not m.startswith('_')]}")
