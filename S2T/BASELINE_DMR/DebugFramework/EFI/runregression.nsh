...
# Initialize DRG_LOOPS_COUNT functionality
if "%DRG_LOOPS_COUNT%" eq "" then
    set DRG_LOOPS_COUNT "NONE"
endif

# If DRG_LOOPS_COUNT is set, disable DRG_LOOP_FOREVER
if "%DRG_LOOPS_COUNT%" neq "NONE" then
    set DRG_LOOP_FOREVER "0"
    echo N-times loop enabled: Will run %DRG_LOOPS_COUNT% times
    if "%3" neq "" then
        echo WARNING: Multiple filters detected. Only first filter will be used in loop mode.
    endif
else
    echo N-times loop disabled (DRG_LOOPS_COUNT not set)
endif

# Check for forever loop with multiple filters warning
if %DRG_LOOP_FOREVER% == 1 then
    if "%3" neq "" then
        echo WARNING: Multiple filters detected. Only first filter will be used in forever loop mode.
    endif
endif
...
if "%VVAR4%" neq "" then
   if "%VVAR5%" eq "" then
      ...
      set VVAR "0 %VVAR0% 1 %VVAR1% 2 %VVAR2% 3 %VVAR3% 4 %VVAR4% 5 %VVAR5% %VVAR_EXTRA%"
else
   set VVAR "0 %VVAR0% 1 %VVAR1% 2 %VVAR2% 3 %VVAR3% %VVAR_EXTRA%"
endif
...
# Check if N-times loop is enabled
if "%DRG_LOOPS_COUNT%" neq "NONE" then
    goto n_times_loop
else
    goto main_loop
...
# =============================================================================
# N-TIMES LOOP SECTION
# =============================================================================

:n_times_loop
for %i run (1 %DRG_LOOPS_COUNT%)
    echo Starting loop iteration %i/%DRG_LOOPS_COUNT%
    goto run_regression_cycle
    :continue_n_times_loop
endfor
goto test_passed

# =============================================================================
# MAIN LOOP SECTION (FOREVER OR SINGLE RUN)
# =============================================================================
...
# Handle looping and multiple filters (applies to first filter only in loop modes)
if %DRG_LOOP_FOREVER% == 1 then
    goto main_loop
endif

# Multiple Filters option (only when not in loop mode)
:filter_loop
...
# =============================================================================
# REGRESSION CYCLE (COMMON FOR BOTH LOOP TYPES)
# =============================================================================
...
if not %2 == "" then
 ...
 echo " "
 # Only shift in single-run mode (not in loop modes)
 if "%DRG_LOOPS_COUNT%" eq "NONE" then
     if %DRG_LOOP_FOREVER% == 0 then
         shift
     endif
 endif
else
...
for %n in %RE%.obj
    ...
    echo Running %n
    echo Running "%MERLIN_COMMAND% -otl -x2ar -a %n -d %VVAR%"
    echo Running "%MERLIN_COMMAND% -otl -x2ar -a %n -d %VVAR%" >> %OBJ_DIR%\log.txt
    echo Running %n >> %n.hng 
    %MERLIN_COMMAND% -otl -x2ar -a %n -d %VVAR%
    rm %n.hng
...
# Return to appropriate loop continuation point
if "%DRG_LOOPS_COUNT%" neq "NONE" then
    goto continue_n_times_loop
else
...
echo  " DRG_LOOPS_COUNT        - Run N times then stop (default: NONE, disables DRG_LOOP_FOREVER when set)"
...
echo  " VVAR4                  - Dragon VVAR 4: Min APIC ID input (optional -default: 0x0)"
echo  " VVAR5                  - Dragon VVAR 5: Max APIC ID (optional -default: 0x7FFFFFFF)"
echo  " VVAR_EXTRA             - Additional VVAR parameters"
echo  " "
echo "IMPORTANT NOTES:"
echo  " - Multiple filters are NOT supported in loop modes (DRG_LOOPS_COUNT or DRG_LOOP_FOREVER)"
echo  " - When loop modes are enabled, only the first filter will be used"
echo  " - Multiple filters only work in single-run mode (no looping)"
echo  " "
echo EXAMPLES:
echo   "runregression.nsh seeds\"
echo   "runregression.nsh seeds\ test1 test2"
echo   "set DRG_STOP_ON_FAIL 1 ^& runregression.nsh seeds\"
echo   "set DRG_LOOPS_COUNT 5 ^& runregression.nsh seeds\"
echo   "set DRG_LOOPS_COUNT 3 ^& runregression.nsh seeds\ filter1"
echo   "set DRG_LOOP_FOREVER 1 ^& runregression.nsh seeds\ filter1"
echo  " "
goto done
...
if "%DRG_LOOPS_COUNT%" neq "" then 
    echo "DRG_LOOPS_COUNT removed"
    set DRG_LOOPS_COUNT ""
else
    echo "DRG_LOOPS_COUNT not set"
endif
