...
def mca_dump(sv, itp, verbose=True):
	...
	with itp.device_locker():
		...
		try:
			for i in sv.socket0.imh0.ubox.ncdecs.biosnonstickyscratchpad_mem:
				mc.append(i)
		except:
			print('ERROR: skipping UBOX scratchpad read, not working')

	pysvdecode = {'ccf':False, 'cha':False, 'llc':False, 'ubox':False, 'punit':False, 'pm':False, 'core':False  }
	if mc != []:
		for i in mc:
			data_path = _get_data_path(i)
			data_value = i.get_value()
			mcadata['TestName'].append("%s" % data_path)
			mcadata['TestValue'].append("0x%x" % data_value)
	...
	if a != []:
		...
		for i in a:
			...
			if decoded_info:
				print(decoded_info)
			print('-'*80)
			if 'cbregs_all' in i.path: pysvdecode['ccf'] = True
...
def _get_data_path(obj):
	"""
	Get the full data path of a register object.
	...
	try:
		if "]" in obj.path:
			# Standardize paths with register[0] for exmaple to register_0
			base_path = obj.path.split(']')[0].replace('[','_')
			return base_path
		else:
			return obj.path
	except Exception:
