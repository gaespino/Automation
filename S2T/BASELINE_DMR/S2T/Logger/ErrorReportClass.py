	...
	def _save_registers_to_file(self, config_path, PC, core_masks, cha_masks):
		...
		try:
			with open(config_path, "a") as file:
				file.write("PC\n")
				file.write(f"sv.socket0.imh0.ubox.ncdecs.biosnonstickyscratchpad_mem[7] = {PC}\n\n") ## add to product config
				...
				for idx in sorted(core_masks.keys()):
					file.write(f"sv.socket0.cbb{idx}.base.fuses.punit_fuses.fw_fuses_sst_pp_0_module_disable_mask.read() = {core_masks[idx]}\n") ## add to product config
				file.write("\n")

				# Write CHA masks
				file.write("CBO Mask\n")
				for idx in sorted(cha_masks.keys()):
					file.write(f"sv.socket0.cbb{idx}.base.fuses.punit_fuses.fw_fuses_llc_slice_ia_ccp_dis.read() = {cha_masks[idx]}\n") ## add to product config
				file.write("\n")
