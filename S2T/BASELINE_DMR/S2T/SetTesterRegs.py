...
class S2TFlow():
	...
	def set_license(self):
		if self.license_level == None and self.__FRAMEWORK_FEATURES['license_level']['enabled']:
			...
			print("\t> 1. SSE/128")
			print("\t> 2. AVX2/256 Heavy")
			print("\t> 3. AVX3/512 Heavy")
			print("\t> 4. TMUL Heavy")


			while self.license_level not in range (0,5):
				self.license_level = int(input("--> Enter 0-4 : "))
	...
	def set_globals(self, flow = 'core'):
		...
		scm.global_boot_extra=",pwrgoodmethod='usb', pwrgoodport=[1,2], pwrgooddelay=45 , disable_mask_checker=True"
...
def vvars_call(slice = True, logCore=None, corestring = 'CORE'):
	...
	if slice:
		...
		else: "\t> VVAR4/5 = HTDisabled Error"
		print(f'{bullets} Use Merlin version 8.23 ---> Individual seed cmd example:')
