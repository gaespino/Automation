...
class FrameworkReportBuilder:
	def __init__(self, root, default_product="GNR"):
		...
		# If default product is set and valid, trigger folder search
		if self.default_product and self.default_product in self.products:
			self.update_visuals_combobox(None)
	...
	def setup_ui(self):
		...
		self.product_var = tk.StringVar(value=self.default_product if self.default_product in self.products else '')
